...
@interface BIMSelectedSaiGuoTitleView()
...
@end
@implementation BIMSelectedSaiGuoTitleView
...
- (void)setArrData:(NSArray *)arrData
{
    _arrData = arrData;
    if (_arrData.count > 0) {
        BIMQiciModel *qici = [_arrData objectAtIndex:_currentIndex + 1];
        if (_isSaiguo) {
...
{
    if (_arrData.count > 0 ) {
        BIMQiciModel *qici = [_arrData objectAtIndex:_currentIndex];
        [_btnDate setTitle:qici.name forState:UIControlStateNormal];
        BIMQiciModel *qiciNext;
        BIMQiciModel *qiciBefore;
        if (_isSaiguo) {
            if (_currentIndex > 0 ) {
