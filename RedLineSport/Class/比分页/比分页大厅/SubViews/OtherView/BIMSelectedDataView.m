...
@interface BIMSelectedDataView()<UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate>
@property (nonatomic, strong) UITableView *tableView;
@end
@implementation BIMSelectedDataView
...
{
    if (_delegate && [_delegate respondsToSelector:@selector(BIMSelecterMatchView:selectedAtIndex:WithSelectedName:)]) {
        if (_arrData.count >0) {
            BIMQiciModel *qici = [_arrData objectAtIndex:indexPath.row];
            [_delegate BIMSelecterMatchView:self selectedAtIndex:indexPath.row WithSelectedName:qici.name];
        }
    }
}
@end
