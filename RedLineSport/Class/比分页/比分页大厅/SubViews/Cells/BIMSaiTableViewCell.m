...
@implementation BIMSaiTableViewCell
...
- (void)setScoreModel:(BIMLiveScoreModel *)ScoreModel
{
    ...
    _eventLab.textColor = [BIMMethods getColor:_ScoreModel.leagueColor];
    ...
    if ([arrPeilv containsObject:@"8002"] && [arrPeilv containsObject:@"8001"]) {
        ...
        if (arrletgoal.count==3) {
            ...
            for (int i = 0; i<arr.count; i++) {
                ...
                if ([BIMMethods isPureInt:str]) {
                    str = [NSString stringWithFormat:@"%d",[str intValue]];
                }else if ([BIMMethods isPureFloat:str]){
    ...
    switch (_ScoreModel.matchstate) {
        ...
        case 1:
        {
            NSString *timeCha =[BIMMethods intervalFromLastDate:_ScoreModel.matchtime2 toTheDate:time];
        ...
        case 3:
        {
            NSString *timeCha = [BIMMethods intervalFromLastDateAnd45:_ScoreModel.matchtime2 toTheDate:time];
...
- (void)attention:(UIButton *)btn {
    ...
    [[BIMDCHttpRequest shareInstance] sendRequestByMethod:@"post" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server, btn.isSelected? url_cancle_attention:url_attention] ArrayFile:nil Start:^(id requestOrignal) {
