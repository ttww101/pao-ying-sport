...
#import "BIMJishiViewController.h"
#import "BIMSaiguoViewController.h"
#import "BIMSaichengViewController.h"
#import "BIMGuanzhuViewController.h"
#import "BIMHSJFoldHeaderView.h"
#import "BIMWebView.h"
#import "BIMSaiTableViewCell.h"
#import "BIMLiveScoreModel.h"
#import "BIMQiciModel.h"
#import "BIMDCJishiBIifenView.h"
#import "BIMDCPlaySound.h"
...
@interface BIMJishiViewController ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate,DCindexBtnDelegate,FoldSectionHeaderViewDelegate, GQWebViewDelegate>
...
@property (nonatomic, strong) BIMDCJishiBIifenView *jsbfView;
...
@property (nonatomic , strong) BIMWebView *activityWeb;
...
@end
@implementation BIMJishiViewController
...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        [_tableView registerClass:[BIMSaiTableViewCell class] forCellReuseIdentifier:cellJishiViewController];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"kaisaisaishi"]) {
        BIMJSbifenModel *model = [_arrData objectAtIndex:section];
        if (model.data.count == 0) {
            ...
            lab.textColor = [BIMMethods getColor:model.leagueColor];
...
{
    BIMSaiTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellJishiViewController];
    if (!cell) {
        cell = [[BIMSaiTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellJishiViewController];
    }
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
   
    BIMLiveScoreModel *model;
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"kaisaisaishi"]) {
        BIMJSbifenModel *bifenModel = [_arrData objectAtIndex:indexPath.section];
...
{
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"kaisaisaishi"]) {
        BIMJSbifenModel *jsmodel = [_arrData objectAtIndex:indexPath.section];
        BIMLiveScoreModel *model = [jsmodel.data objectAtIndex:indexPath.row];
...
{
    if (!_indexBtn) {
        _indexBtn = [[BIMDCindexBtn alloc] initWithFrame:CGRectMake(Width - 50, APPDELEGATE.customTabbar.height_myNavigationBar + 44 + 29, 50, Height - (APPDELEGATE.customTabbar.height_myNavigationBar + 44 + 29 + 49))];
...
//        NSString *uBIMtage = @"http://120.55.30.173:8809/bifen/live";
        NSString *uBIMtage = [NSString stringWithFormat:@"%@/bifen2/live",APPDELEGATE.url_Server];
        ...
        dispatch_async(concurrentqueue, ^{
            [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:uBIMtage Start:^(id requestOrignal) {
                ...
                NSMutableArray *arrLoad = [[NSMutableArray alloc] initWithArray:[BIMJSbifenModel arrayOfEntitiesFromArray:[[responseOrignal objectForKey:@"data"] objectForKey:@"matchs"]]];
                
                if (_memeryArrAllPart.count > 0) {
                    if ([[[_memeryArrAllPart firstObject] class]isEqual:NSClassFromString(@"BIMLiveScoreModel")]) {
                        [_memeryArrAllPart removeAllObjects];
                    }
                }
                ...
                for (int i = 0; i<_memeryArrAllPart.count; i++) {
                    BIMJSbifenModel *jsmodel = [_memeryArrAllPart objectAtIndex:i];
                    for (int m= 0; m<jsmodel.data.count; m++) {
                        BIMLiveScoreModel *model = [jsmodel.data objectAtIndex:m];
                        if (model.matchstate != 0){
                            for (int j= 0; j<arrLoad.count; j++) {
                                BIMJSbifenModel *jsloadmodel = [arrLoad objectAtIndex:j];
                                for (int n = 0; n<jsloadmodel.data.count; n++) {
                                    BIMLiveScoreModel *loadModel = [jsloadmodel.data objectAtIndex:n];
                ...
                for (BIMLiveScoreModel *completeModel in arrComplete) {
                    
                    for (int i = 0; i<arrLoad.count; i++) {
                        
                        BIMJSbifenModel *jsmodel = [arrLoad objectAtIndex:i];
                ...
                BIMJSbifenModel *lastModel = [arrLoad lastObject];
        ...
        dispatch_async(concurrentqueue, ^{
            [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:uBIMtage Start:^(id requestOrignal) {
                ...
                if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
                    NSMutableArray *arrLoad = [[NSMutableArray alloc] initWithArray:[BIMLiveScoreModel arrayOfEntitiesFromArray:[[responseOrignal objectForKey:@"data"] objectForKey:@"matchs"]]];
                    
                    if (_memeryArrAllPart.count > 0) {
                        if ([[[_memeryArrAllPart firstObject] class]isEqual:NSClassFromString(@"BIMJSbifenModel")]) {
                            [_memeryArrAllPart removeAllObjects];
                        }
                    }
                    ...
                    for (int m= 0; m < _memeryArrAllPart.count; m++) {
                        BIMLiveScoreModel *model = [_memeryArrAllPart objectAtIndex:m];
                        if (model.matchstate != 0){
                            for (int n = 0; n<arrLoad.count; n++) {
                                BIMLiveScoreModel *loadModel = [arrLoad objectAtIndex:n];
                                if (model.mid == loadModel.mid) {
                                    if (loadModel.matchstate != model.matchstate) {
                                        if (!(loadModel.matchstate==0 ||loadModel.matchstate==1 ||loadModel.matchstate==2 ||loadModel.matchstate==3 ||
                                              loadModel.matchstate==4)) {
...
{
    if (!_hud) {
        _hud = [[MBProgressHUD alloc] initWithView:[BIMMethods getMainWindow]];
        ...
        [[BIMMethods getMainWindow] addSubview:_hud];
    }
    return _hud;
}
- (BIMDCJishiBIifenView *)jsbfView
{
    if (!_jsbfView) {
        _jsbfView = [[BIMDCJishiBIifenView alloc] initWithFrame:CGRectMake(20, Height - 49- 30 - 66, Width - 40, 66)];
        BIMJsbfValue *value = [[BIMJsbfValue alloc] init];
...
- (void)changeLivingScoreWithData:(NSArray *)arrLiving
{
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"kaisaisaishi"]) {
        ...
        if (_memeryArrAllPart.count > 0) {
            if ([[[_memeryArrAllPart firstObject] class]isEqual:[BIMLiveScoreModel class]]) {
                [self getNewData];
                return;
            }
        }
        ...
        if (_memeryArrAllPart.count > 0) {
            if ([[[_memeryArrAllPart firstObject] class] isEqual:[BIMJSbifenModel class]]) {
                [self getNewData];
                return;
            }
        ...
        BIMJSbifenModel *model = [[BIMJSbifenModel alloc]init];
    ...
    if (appendArray > 0 ) {
        for (int i = 0 ; i<appendArray.count; i++) {
            BIMJSbifenModel *jsmodel = [appendArray objectAtIndex:i];
            for (int m = 0; m<jsmodel.data.count; m++) {
                BIMLiveScoreModel * scoreModel= [jsmodel.data objectAtIndex:m];
                for (int j= 0; j<arr.count; j++){
                    BIMLivingModel *livingModel = [arr objectAtIndex:j];
                    if (scoreModel.mid == livingModel.sid) {
                        ...
                        NSString *documentsPath = [BIMMethods getDocumentsPath];
                        ...
                        if (![[[NSUserDefaults standardUserDefaults] objectForKey:@"_currentflag"] isEqualToString:@"0"]) {
                            if (scoreModel.neutrality) {
                                BIMLivingModel *change = [[BIMLivingModel alloc] init];
                        ...
                        {
                            if ([[[BIMMethods getDateByDate:[NSDate date] withWeekType:weekTypeXingqi]objectAtIndex:3] intValue]>=7 && [[[BIMMethods getDateByDate:[NSDate date] withWeekType:weekTypeXingqi]objectAtIndex:3] intValue]<=23)
                            {
        ...
        for (int m = 0; m<jsmodel.data.count; m++) {
            ...
            if (timeType) {
                for (int j= 0; j<_memeryArrAllPart.count; j++){
                    BIMLiveScoreModel *mmModel = [_memeryArrAllPart objectAtIndex:j];
                ...
                for (int j= 0; j<_memeryArrAllPart.count; j++){
                    BIMJSbifenModel *jsMMmodel = [_memeryArrAllPart objectAtIndex:j];
                    for (int n = 0; n<jsMMmodel.data.count; n++) {
                        BIMLiveScoreModel *mmModel = [jsMMmodel.data objectAtIndex:n];
                        if (scoreModel.mid == mmModel.mid) {
                            mmModel = scoreModel;
                        }
                    }
    ...
    NSMutableArray *arrRemove = [NSMutableArray array];
            for (BIMLiveScoreModel *model in arrComplete) {
                for (int i = 0; i<appendArray.count; i++) {
                    BIMJSbifenModel *jsmodel = [appendArray objectAtIndex:i];
    ...
    BIMJSbifenModel *jsmodel= [appendArray lastObject];
...
- (void)jinqiuShowHudWithLivingModel:(BIMLivingModel *)livingModel NowModel:(BIMLiveScoreModel *)scoreModel
{
    if (livingModel.hsc > scoreModel.homescore) {
        scoreModel.homescore =livingModel.hsc;
        BIMJsbfValue *model = [[BIMJsbfValue alloc] init];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([[NSUserDefaults standardUserDefaults] boolForKey:@"jinqiutanchuan"] || [[NSUserDefaults standardUserDefaults] boolForKey:@"hongpaitanchuang"]) {
                [[BIMMethods getMainWindow] addSubview:self.jsbfView];
            }
...
- (void)hongpaiShowHudLivingModel:(BIMLivingModel *)livingModel NowModel:(BIMLiveScoreModel *)scoreModel
{
...
- (NSString *)getLivetime:(BIMLiveScoreModel *)scoreModel
{
    NSString *time = [BIMMethods getDateByStyle:dateStyleFormatter withDate:[NSDate date]];
    switch (scoreModel.matchstate) {
        ...
        case 1:
        {
            NSString *timeCha =[BIMMethods intervalFromLastDate:scoreModel.matchtime2 toTheDate:time];
        ...
        case 3:
        {
            NSString *timeCha = [BIMMethods intervalFromLastDateAnd45:scoreModel.matchtime2 toTheDate:time];
        ...
        case 4:
        {
            return  [BIMMethods intervalFromLastDateAnd45:scoreModel.matchtime2 toTheDate:time];
        }
            break;
        default:
        {
...
- (void)loadRedBombActivity {
    NSMutableDictionary *parameter = [NSMutableDictionary dictionaryWithDictionary:[BIMHttpString getCommenParemeter]];
    [[BIMDCHttpRequest shareInstance]sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_redBomb]  Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
...
- (void)redBombActivity:(UIGestureRecognizer *)tap {
    if (self.activityDic) {
        ...
        BIMWebView *web = [[BIMWebView alloc]init];
        ...
        [[BIMMethods getMainWindow] addSubview:web];
        _activityWeb = web;
    } else {
