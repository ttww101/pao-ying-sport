...
#import "BIMBifenSetingViewController.h"
#import "BIMSearchMatchVC.h"
...
#import "BIMSelecterMatchView.h"
#import "BIMSelectedEventView.h"
...
@interface BIMBifenViewController ()<ViewPagerDataSource,ViewPagerDelegate,SelectedEventViewDelegate,NavViewDelegate,SelecterMatchViewDelegate,HSTabBarContentViewDelegate, GeneralFloatingViewDelegate, GBMenuButtonDelegate>
@property (nonatomic, strong)BIMJishiViewController *jishiVC;
@property (nonatomic, strong)BIMSaiguoViewController *saiguoVC;
@property (nonatomic, strong)BIMSaichengViewController *saichengVC;
@property (nonatomic, strong)BIMGuanzhuViewController *guanzhuVC;
@property (nonatomic, strong) BIMSelectedEventView*titleView;
...
@property (nonatomic, strong) BIMSelecterMatchView *matchView;
...
@end
@implementation BIMBifenViewController
...
- (void)btnSearch
{
    BIMSearchMatchVC *searchVC = [[BIMSearchMatchVC alloc] init];
...
- (void)tabBarContentView:(BIMHSTabBarContentView *)tabBarContentView didSelectItemAtIndex:(NSInteger)index {
    ...
    [self BIMSelecterMatchView:nil selectedAtIndex:index];
}
...
- (void)navViewTouchAnIndex:(NSInteger)index
{
    if (index == 1) {
        [[NSUserDefaults standardUserDefaults] setBool:NO forKey:@"showJinqiuAnimation"];
        BIMBifenSetingViewController *biFenSetVC = [[BIMBifenSetingViewController alloc] init];
...
- (BIMSelecterMatchView *)matchView
{
    if (!_matchView) {
        _matchView = [[BIMSelecterMatchView alloc] initWithFrame:CGRectMake(0, APPDELEGATE.customTabbar.height_myNavigationBar, Width, Height - APPDELEGATE.customTabbar.height_myNavigationBar)];
        _matchView.hidden = YES;
        _matchView.alpha = 0;
...
- (BIMSelectedEventView *)titleView{
    if (!_titleView) {
        _titleView = [[BIMSelectedEventView alloc] initWithFrame:CGRectMake(0, APPDELEGATE.customTabbar.height_myNavigationBar+60+10, Width, 44)];
...
- (BIMJishiViewController *)jishiVC
{
    if (!_jishiVC) {
        _jishiVC = [BIMJishiViewController new];
    }
    return  _jishiVC;
}
- (BIMSaiguoViewController *)saiguoVC
{
    if (!_saiguoVC) {
        _saiguoVC = [BIMSaiguoViewController new];
    }
    return  _saiguoVC;
}
- (BIMSaichengViewController *)saichengVC
{
    if (!_saichengVC) {
        _saichengVC = [BIMSaichengViewController new];
    }
    return _saichengVC;
}
- (BIMGuanzhuViewController *)guanzhuVC
{
    if (!_guanzhuVC) {
        _guanzhuVC = [BIMGuanzhuViewController new];
    }
    return _guanzhuVC;
}
