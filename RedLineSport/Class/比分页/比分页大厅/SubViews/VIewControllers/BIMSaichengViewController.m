...
@interface BIMSaichengViewController ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate,SelecterDateViewDelegate,SelectedDateTitleViewDelegate,DCindexBtnDelegate,HSInfiniteScrollViewDataSource, DatePickerViewDelegate>
...
@property (nonatomic, strong) BIMHSInfiniteScrollView            *titleScrollView;
...
@end
@implementation BIMSaichengViewController
...
# pragma mark - BIMHSInfiniteScrollView dataSource
...
# pragma mark - BIMHSInfiniteScrollView delegate -
- (UIView *)viewAtIndex:(NSInteger)index reusingView:(UIView *)view {
    ...
    BIMQiciModel *model = [_arrDataQici objectAtIndex:index];
...
- (void)scrollView:(BIMHSInfiniteScrollView *)scrollView didScrollToIndex:(NSInteger)index {
    NSLog(@"scroll to: %ld", index);
}
- (void)viewTap: (UIGestureRecognizer *)ges {
    [self BIMSelecterMatchView:nil selectedAtIndex:ges.view.tag WithSelectedName:nil];
}
- (BIMHSInfiniteScrollView *)titleScrollView {
    if (!_titleScrollView) {
        _titleScrollView = [[BIMHSInfiniteScrollView alloc] initWithFrame:CGRectMake(0, APPDELEGATE.customTabbar.height_myNavigationBar + 44+60, Width, self.seletedHeight)];
    }
...
{
    if (!_dataTitleView) {
        _dataTitleView = [[BIMSelectedDateTitleView alloc] initWithFrame:CGRectMake(0, APPDELEGATE.customTabbar.height_myNavigationBar + 44+60+14, Width, 60 / 2 + 9)];
...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        [_tableView registerClass:[BIMSaiTableViewCell class] forCellReuseIdentifier:cellSaichengViewController];
...
{
    BIMSaiTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellSaichengViewController];
    if (!cell) {
        cell = [[BIMSaiTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellSaichengViewController];
    }
