...
@interface BIMGuanzhuViewController ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate, SelectedDateTitleViewDelegate, DatePickerViewDelegate>
...
@implementation BIMGuanzhuViewController
...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        [_tableView registerClass:[BIMSaiTableViewCell class] forCellReuseIdentifier:cellGuanzhuViewController];
...
{
    BIMSaiTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellGuanzhuViewController];
    if (!cell) {
        cell = [[BIMSaiTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellGuanzhuViewController];
    }
...
- (void)getAttention
{
    ...
    [[BIMDCHttpRequest shareInstance] sendRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_focusd_matches] ArrayFile:nil Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            _arrData = [[NSMutableArray alloc] initWithArray:[BIMLiveScoreModel arrayOfEntitiesFromArray:[[responseOrignal objectForKey:@"data"] objectForKey:@"matches"]]];
            ...
            for (NSInteger i = 0; i < _arrData.count; i ++) {
                BIMLiveScoreModel *model = _arrData[i];
