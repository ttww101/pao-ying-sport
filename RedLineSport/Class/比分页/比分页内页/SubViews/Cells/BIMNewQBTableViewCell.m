...
@interface BIMNewQBTableViewCell ()
...
@end
@implementation BIMNewQBTableViewCell
- (void)awakeFromNib {
    [super awakeFromNib];
}
...
- (void)setModel:(BIMInfoListModel *)model
{
    _model = model;
    ...
    _labHomeOrAway.textColor = [BIMMethods getColor:model.newsTypeColor];
    _labtitle.text = [NSString stringWithFormat:@"%@",_model.title];
    _bestQingbao.hidden = !model.best;
    if (_model.content == nil) {
        _model.content = @"";
    }
    [_labContent setAttributedText:[BIMMethods setTextStyleWithString:_model.content WithLineSpace:6 WithHeaderIndent:heightHeaderIndent]];
    if (_hideBottomView) {
