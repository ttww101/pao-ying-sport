...
#import "BIMshowVideoView.h"
#import "BIMShowSignals.h"
@interface BIMFenxiHeaderView () <BIMShowSignalsDelegate>
{
}
...
@property (nonatomic , strong) BIMShowSignals *choiceSignalView;

@end
@implementation BIMFenxiHeaderView
...
- (void)updateScroeWithmodel:(BIMLiveScoreModel *)liviModel{
    ...
    if (liviModel.matchstate == -1) {
        ...
        NSString *timeCha =[BIMMethods intervalFromLastDate:liviModel.matchtime2 toTheDate:time];
        ...
        NSString *timeCha =[BIMMethods intervalFromLastDateAnd45:liviModel.matchtime2 toTheDate:time];
        ...
        _labState.text = [BIMMethods getTextByMatchState:liviModel.matchstate];
        _labHscore.text = @"VS";
    }
}
- (void)setModel:(BIMLiveScoreModel *)model{
    ...
    _labTime.text = [BIMMethods getDateByStyle:dateStyleFormatterMdHm withDate:[BIMMethods getDateFromString:_model.matchtime byformat:dateStyleFormatter]];
    ...
    if (_model.matchstate == -1) {
        ...
        NSString *timeCha =[BIMMethods intervalFromLastDate:_model.matchtime2 toTheDate:time];
        if ([timeCha isEqualToString:@"0"]) {
            _labState.text = @"1";
        }else{
            if ([timeCha intValue]>45) {
        ...
        NSString *timeCha =[BIMMethods intervalFromLastDateAnd45:_model.matchtime2 toTheDate:time];
        if ([timeCha isEqualToString:@"45"]) {
            _labState.text = @"46";
        }else{
            if ([timeCha intValue]>90) {
        ...
        _labState.text = [BIMMethods getTextByMatchState:_model.matchstate];
        _labHscore.text = @"VS";
    }
...
#pragma mark - BIMShowSignalsDelegate

- (void)didSelectItem:(NSDictionary *)data {
    NSString *url = data[@"url"];
    if ([data[@"type"] integerValue] == 2) {
        if (url.length > 0) {
            BIMshowVideoView *video = [[BIMshowVideoView alloc]init];
            video.url = url;
            [[BIMMethods getMainWindow] addSubview:video];
        }
    } else {
...
- (void)liveAction {
    ...
    if (self.signalLists.count > 1) {
        if (!_choiceSignalView) {
            _choiceSignalView = [[BIMShowSignals alloc]initWithFrame:CGRectMake(0, 0, Width, Height) mid:[NSString stringWithFormat:@"%zi", _model.mid]];
            _choiceSignalView.delegate = self;
            [[BIMMethods help_getCurrentVC].view addSubview:_choiceSignalView];
        }
...
- (void)checkoutVideoSignal {
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        [[BIMDCHttpRequest shareInstance]sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_video_list] Start:^(id requestOrignal) {
