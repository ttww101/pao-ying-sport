...
#import "BIMFenxiHeaderView.h"
#import "PictureModel.h"
#import "BIMFenxiPageVC.h"
#import "BIMBifenDTTable.h"
#import "BIMNewQingbaoTableView.h"
#import "BIMInfoListModel.h"
#import "BIMTuijianDatingTableView.h"
#import "BIMTitleIndexView.h"
#import "BIMLivingModel.h"
#import "BIMTimeModel.h"
#import "BIMNewTuijianHtml.h"
#import "BIMZhiboTableView.h"
#import "BIMJiBenWebView.h"
#import "BIMRelRecNewVC.h"
#import "BIMToAnalystsVC.h"
#import "BIMFenXiHeaderBottomView.h"
#import "SRWebSocket.h"
#import "BIMRecommendedWebView.h"
...
@interface BIMFenxiPageVC ()<UIScrollViewDelegate,NewQingbaoTableViewDelegate,TuijianDatingTableViewDelegate,ViewPagerDelegate,TitleIndexViewDelegate,FenxiHeaderViewDelegate,UIWebViewDelegate,UITableViewDataSource,UITableViewDelegate,SRWebSocketDelegate, NELivePlayerControlViewProtocol, UIGestureRecognizerDelegate>

@property (nonatomic, strong) BIMBifenDTTable *tableView;
@property (nonatomic, strong) BIMTitleIndexView *titleView;
@property (nonatomic, strong) BIMDCScrollVIew *scrollMainView;
@property (nonatomic, strong) NSMutableArray *arrTableViews;
@property (nonatomic, strong) BIMFenXiHeaderBottomView        *hederBottomView;
@property (nonatomic, assign) BOOL hideHeaderView;
@property (nonatomic, strong) BIMFenxiHeaderView *headerView;
@property (nonatomic, strong) BIMNewQingbaoTableView *NewQBTableView;
@property (nonatomic, strong) BIMTuijianDatingTableView *tuiJianTable;
@property (nonatomic, strong) BIMJiBenWebView *webView;
@property (nonatomic, strong) BIMRecommendedWKWeb *webViewZhiShu;
@property (nonatomic, strong) BIMRecommendedWKWeb *recommendWeb;
@property (nonatomic , strong) BIMAnalysisWebview *analysisWeb; 
@property (nonatomic, strong) BIMRecommendedWKWeb *webZhiBo;
...
@property (nonatomic , strong) BIMShowActivityView *animationActivityView;
...
- (void)timerRun
{
    NSDate *matchDate = [BIMMethods getDateFromString:_model.matchtime byformat:dateStyleFormatter];
...
- (void)timerChange
{
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:nil PathUrlL:[NSString stringWithFormat:@"%@%@%@",APPDELEGATE.url_jsonHeader,@"/jsbf",url_jsbf_change] Start:^(id requestOrignal) {
        ...
        for (int i = 0; i<arrLiving.count; i++) {
            BIMLivingModel *living = [arrLiving objectAtIndex:i];
...
- (void)updateHeaderData:(NSNotification *)notific
{
    ...
    for (int i = 0; i<arrfenxiHeader.count; i++) {
        BIMLiveScoreModel *liveModel = [arrfenxiHeader objectAtIndex:i];
...
- (BIMBifenDTTable *)tableView
{
    if (!_tableView) {
        _tableView = [[BIMBifenDTTable alloc] initWithFrame:CGRectMake(0, 0, Width, Height) style:UITableViewStylePlain];
        _tableView.tag = 10;
...
- (BIMFenxiHeaderView *)headerView
{
    if (!_headerView) {
        _headerView = [[BIMFenxiHeaderView alloc] initWithFrame:CGRectMake(0, 0, Width, [(self.model.weather) isEqualToString:@""]? 164 : 190)]; 
...
- (UIView *)hederBottomView {
    if (!_hederBottomView) {
        _hederBottomView = [[BIMFenXiHeaderBottomView alloc] initWithFrame:CGRectMake(0, _headerView.bottom, Width, 40)];
...
- (BIMDCScrollVIew *)scrollMainView
{
    if (!_scrollMainView) {
        _scrollMainView = [[BIMDCScrollVIew alloc] initWithFrame:CGRectMake(0, 0, Width, Height - 44 - APPDELEGATE.customTabbar.height_myNavigationBar)];;
...
- (void)btnFabuCilick:(UIButton *)btn
{
    switch (btn.tag) {
        ...
        case 3:
        {
            ...
            BIMDan_StringMatchsModel *remodel = [BIMDan_StringMatchsModel entityFromDictionary:[NSDictionary dictionary]];
...
- (void)toapplyAnalasis
{
    ...
    if (user.analyst == 1) {
        ...
        remodel.hometeam = _model.hometeam;
        remodel.guestteam = _model.guestteam;
        BIMRelRecNewVC *relVC = [[BIMRelRecNewVC alloc] init];
        relVC.model = remodel;
...
- (BIMNewQingbaoTableView *)NewQBTableView
{
    if (!_NewQBTableView) {
        _NewQBTableView = [[BIMNewQingbaoTableView alloc] initWithFrame:CGRectMake(Width*2, 0, Width, _scrollMainView.height) style:UITableViewStylePlain];
...
- (BIMTuijianDatingTableView *)tuiJianTable
{
    if (!_tuiJianTable) {
        _tuiJianTable = [[BIMTuijianDatingTableView alloc] initWithFrame:CGRectMake(Width*3, 0, Width, _scrollMainView.height) style:UITableViewStylePlain];
...
- (BIMRecommendedWKWeb *)recommendWeb {
    if (_recommendWeb == nil) {
        _recommendWeb = [[BIMRecommendedWKWeb alloc]initWithFrame:CGRectMake(Width*3, 0, Width, _scrollMainView.height)];
...
- (BIMJiBenWebView *)webView{
    if (!_webView) {
        _webView = [[BIMJiBenWebView alloc] initWithFrame:CGRectMake(0, 0, Width, _scrollMainView.height)];
...
- (BIMAnalysisWebview *)analysisWeb {
    if (_analysisWeb == nil) {
        _analysisWeb = [[BIMAnalysisWebview alloc]initWithFrame:CGRectMake(0, 0, Width, _scrollMainView.height)];
        BIMWebModel *model = [[BIMWebModel alloc]init];
...
- (BIMRecommendedWKWeb *)webViewZhiShu {
    if (_webViewZhiShu == nil) {
        _webViewZhiShu = [[BIMRecommendedWKWeb alloc]initWithFrame:CGRectMake(Width, 0, Width, _scrollMainView.height)];
        BIMWebModel *model = [[BIMWebModel alloc]init];
...
- (BIMRecommendedWKWeb *)webZhiBo {
    if (_webZhiBo == nil) {
        _webZhiBo = [[BIMRecommendedWKWeb alloc]initWithFrame:CGRectMake(Width*4, 0, Width, _scrollMainView.height)];
        BIMWebModel *model = [[BIMWebModel alloc]init];
...
- (BIMTitleIndexView *)titleView
{
    if (!_titleView) {
        _titleView = [[BIMTitleIndexView alloc] initWithFrame:CGRectMake(0, self.headerView.bottom, Width, 44)];
...
#pragma mark ----- 请求提点的数据
- (void)lodaDataTiDian {
    NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithDictionary:[BIMHttpString getCommenParemeter]];
    [dict setObject:@(self.model.mid) forKey:@"scheduleId"];
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:dict PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_tidian] Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
                NSLog(@"提点%@",responseOrignal);
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            _NewQBTableView.jiDianArr = [[NSMutableArray alloc] initWithArray:[BIMTimeModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"data"]]];
...
- (void)lodaDataAnalysisQB {
    ...
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:dict PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_infolistOneSchedule] Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            _NewQBTableView.arrhomeInfo = [[NSArray alloc] initWithArray:[BIMInfoListModel arrayOfEntitiesFromArray:[[responseOrignal objectForKey:@"data"] objectForKey:@"homeInfo"]]];
            _NewQBTableView.arrawayInfo = [[NSArray alloc] initWithArray:[BIMInfoListModel arrayOfEntitiesFromArray:[[responseOrignal objectForKey:@"data"] objectForKey:@"awayInfo"]]];
            _NewQBTableView.arrneutralInfo = [[NSArray alloc] initWithArray:[BIMInfoListModel arrayOfEntitiesFromArray:[[responseOrignal objectForKey:@"data"] objectForKey:@"neutralInfo"]]];
...
- (void)uploadImageWithImageArr:(NSArray *)arrImage completion:(void(^)(BOOL finished,NSArray*arrUrl)) completion
{
    ...
    [[BIMDCHttpRequest shareInstance] sendRequestByMethod:@"post" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_ZiXunUrl] ArrayFile:arrImage Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
...
- (BIMShowActivityView *)animationActivityView {
    if (_animationActivityView == nil) {
        _animationActivityView = [[BIMShowActivityView alloc]initWithFrame:CGRectMake(80, 4 * (Height / 5), 80, 80)];
    }
