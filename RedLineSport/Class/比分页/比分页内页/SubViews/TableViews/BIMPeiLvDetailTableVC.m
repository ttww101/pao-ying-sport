...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:[BIMPeiLvDetailCell class] forCellReuseIdentifier:peilvDetailCellID];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    BIMPeiLvDetailCell *peilvCell = [tableView cellForRowAtIndexPath:indexPath];
    if (!peilvCell) {
        peilvCell = [[BIMPeiLvDetailCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:peilvDetailCellID];
    }
    ...
    if (self.PeiLvCDetailType == isDetailBeforeTwo) {
        if (_peiLvDetailArr.count > 0) {
            BIMPeiLvDetailModel *nowModel = _peiLvDetailArr[indexPath.row];
            if (_peiLvDetailArr.count > 1) {
                if (indexPath.row!= _peiLvDetailArr.count-1) {
                    BIMPeiLvDetailModel *nextModel = _peiLvDetailArr[indexPath.row +1];
        ...
        if (_peiLvAfterArr.count > 0) {
            BIMPeiLvDetailModel *nowModel = _peiLvAfterArr[indexPath.row];
            if (_peiLvAfterArr.count > 1) {
                if (indexPath.row!= _peiLvAfterArr.count-1) {
                    BIMPeiLvDetailModel *nextModel = _peiLvAfterArr[indexPath.row +1];
                    if (![nowModel.Score isEqualToString:nextModel.Score]) {
                        nowModel.ischangedScore = YES;
                    }
                    if ([nowModel.HomeOdds floatValue]>[nextModel.HomeOdds floatValue]) {
...
- (void)loadPeiLvDataWithScheid:(NSInteger)scheid OddsType:(oddsType)oddsType isHalf:(isHalfType)isHalf {
    ...
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:nil PathUrlL:lsURl Start:^(id requestOrignal) {
        ...
        _peiLvDetailArr = [[NSMutableArray alloc] initWithArray:[BIMPeiLvDetailModel arrayOfEntitiesFromArray:responseOrignal]];
        if (_peiLvDetailArr.count>0) {
            BIMPeiLvDetailModel*model=_peiLvDetailArr[_peiLvDetailArr.count-1];
            model.BlackType=1;
