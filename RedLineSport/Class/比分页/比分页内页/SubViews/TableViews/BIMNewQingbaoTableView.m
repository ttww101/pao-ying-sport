...
#import "BIMNewQBTableViewCell.h"
#import "BIMTimeModel.h"
#import "BIMRightSlidetabletableViewCell.h"
#import "LockQingBaoView.h"
#import "BIMToolWebViewController.h"


static NSString * iden = @"testTime";
@interface BIMNewQingbaoTableView ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate, LockQingBaoViewDelegate>
@property (nonatomic, assign) CGFloat oldContentY;
@property(strong,nonatomic)NSMutableArray * dataList;
@property (nonatomic , strong) LockQingBaoView *lockView;

@end
@implementation BIMNewQingbaoTableView
...
- (void)setup
{
    ...
    [self registerClass:[BIMNewQBTableViewCell class] forCellReuseIdentifier:cellNewQB];
    [self registerClass:[BIMRightSlidetabletableViewCell class] forCellReuseIdentifier:iden];
...
- (CGFloat) tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    switch (indexPath.section) {
        case 0:
        {
            BIMInfoListModel *infoModel = [self.arrhomeInfo objectAtIndex:indexPath.row];
            CGFloat heiContent = [BIMMethods getTextHeightStationWidth:[NSString stringWithFormat:@"%@",infoModel.content] anWidthTxtt:Width - 30 anfont:14 andLineSpace:5.5 andHeaderIndent:0];
            CGFloat heiTitle = [BIMMethods getTextHeightStationWidth:infoModel.title anWidthTxtt:Width - 30 anfont:20 andLineSpace:5 andHeaderIndent:0];
        ...
        case 1:
        {
            BIMInfoListModel *infoModel = [self.arrneutralInfo objectAtIndex:indexPath.row];
            CGFloat heiContent = [BIMMethods getTextHeightStationWidth:[NSString stringWithFormat:@"%@",infoModel.content] anWidthTxtt:Width - 30 anfont:14 andLineSpace:5.5 andHeaderIndent:0];
            CGFloat heiTitle = [BIMMethods getTextHeightStationWidth:infoModel.title anWidthTxtt:Width - 30 anfont:20 andLineSpace:5 andHeaderIndent:0];
        ...
        case 2:
        {
            BIMInfoListModel *infoModel = [self.arrawayInfo objectAtIndex:indexPath.row];
            CGFloat heiContent = [BIMMethods getTextHeightStationWidth:[NSString stringWithFormat:@"%@",infoModel.content] anWidthTxtt:Width - 30 anfont:14 andLineSpace:5.5 andHeaderIndent:0];
            CGFloat heiTitle = [BIMMethods getTextHeightStationWidth:infoModel.title anWidthTxtt:Width - 30 anfont:20 andLineSpace:5 andHeaderIndent:0];
            return 60 + heiContent + heiTitle;
        }
        ...
        case 3:
        {
            BIMTimeModel * model = self.jiDianArr[indexPath.row];
...
{
    BIMNewQBTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellNewQB];
    if (!cell) {
        cell = [[BIMNewQBTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellNewQB];
    }
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
        switch (indexPath.section) {
            ...
            case 3:
            {
                BIMRightSlidetabletableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:iden];
                if (!cell) {
                    cell = [[BIMRightSlidetabletableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:iden];
                }
                    BIMTimeModel * timemodel = self.dataList[indexPath.row];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    switch (section) {
        case 0:
        {
            BIMInfoListModel *infoModel = [self.arrhomeInfo objectAtIndex:0];
        ...
        case 2:
        {
            BIMInfoListModel *infoModel = [self.arrawayInfo objectAtIndex:0];
            UILabel *labName =  [[UILabel alloc] initWithFrame:CGRectMake(15, 0, Width - 104, 35)];
            labName.font = font14;
