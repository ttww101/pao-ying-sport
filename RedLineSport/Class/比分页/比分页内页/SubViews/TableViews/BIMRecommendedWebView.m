...
#import "BIMWebviewProgressLine.h"
#import <WebKit/WebKit.h>
@interface BIMRecommendedWebView () <UIWebViewDelegate, WKUIDelegate, WKNavigationDelegate, UIWebViewDelegate>
@property (nonatomic , copy) GQJSResponseCallback callBack;
@property (nonatomic , strong) BIMWebviewProgressLine *progressLine;
@property (nonatomic , strong) WebViewJavascriptBridge* bridge;
@end
@implementation BIMRecommendedWebView

- (instancetype)initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if (self) {
        self.backgroundColor = colorTableViewBackgroundColor;
        self.progressLine = [[BIMWebviewProgressLine alloc] initWithFrame:CGRectMake(0, 0, Width, 3)];
...
- (void)setModel:(BIMWebModel *)model {
    _model = model;
    [self loadData];
}
-(void)downLoadWeb {
...
- (void)loadBradgeHandler {
    __weak BIMRecommendedWebView *weakSelf = self;
...
- (void)openNative:(id)data {
    if ([data isKindOfClass:[NSDictionary class]]) {
        ...
        if (target == nil) {
            ...
            [[BIMMethods help_getCurrentVC].navigationController pushViewController:target animated:YES];
        }
    }
}
