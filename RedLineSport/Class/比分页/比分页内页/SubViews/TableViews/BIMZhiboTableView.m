#import "BIMZhiboTableView.h"
#import "BIMLiveEventMedel.h"
#import "BIMliveLineupModel.h"
#import "BIMTechModel.h"
#import "BIMTechtwoModel.h"
#import "BIMJishiFirstCell.h"
#import "BIMJiShiPeiLvCell.h"
#import "BIMPeiLvDetailCell.h"
#import "BIMJiShiPeiLvDetailModel.h"
#import "BIMLiveScoreModel.h"
#import "BIMJiShiPeiLvDetailModel.h"
#import "BIMDoubleBattlecell.h"
#import "BIMPeiLvDetailVC.h"
#import "BIMHSJFoldHeaderView.h"
#import "BIMJSPLDownCell.h"
#import "BIMJSPLDownMode.h"
#import "BIMBisaiTongjiCell.h"
#import "BIMBisaiTongjiCellTwo.h"
#import "BIMBisaiTJHeaderView.h"
#import "BIMBattleModel.h"
#import "BIMJSPLDownMode.h"
#import "BIMJSPLDownTwoModel.h"
#import "BIMTongJiModel.h"
#define jiShiPeilvDetailCellID         @"jiShiPeilvDetailCell"
#define jiShiFirstCellID               @"jiShiPeiLvFirstCell"
#define doubleBattleCell               @"BIMDoubleBattlecell"
...
@interface BIMZhiboTableView()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate,FoldSectionHeaderViewDelegate,SRWebSocketDelegate>
{
    NSTimer* _timer;
    NSInteger num;
}
...
@property (nonatomic, strong) BIMJSPLDownMode *jsplDownone;
@property (nonatomic, strong) BIMJSPLDownTwoModel *jsplDownTwo;
...
@end
@implementation BIMZhiboTableView
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        [self registerClass:[BIMJishiFirstCell class] forCellReuseIdentifier:jiShiFirstCellID];
        [self registerClass:[BIMJiShiPeiLvCell class] forCellReuseIdentifier:jiShiPeilvDetailCellID];
        [self registerClass:[BIMDoubleBattlecell class] forCellReuseIdentifier:doubleBattleCell];
        [self registerClass:[BIMJSPLDownCell class] forCellReuseIdentifier:jSPLDownCelliD];
        [self registerClass:[BIMBisaiTongjiCell class] forCellReuseIdentifier:cellBisaiTongji];
        [self registerClass:[BIMBisaiTongjiCellTwo class] forCellReuseIdentifier:cellBisaiTongjiTwo];
        self.delegate =self;
...
- (void)setModel:(BIMLiveScoreModel *)model
{
    _model = model;
    NSMutableArray *arrEvent = [NSMutableArray array];
    BIMLiveEventMedel *start = [[BIMLiveEventMedel alloc] init];
    start.headerOrFooter = 1;
    BIMLiveEventMedel *end = [[BIMLiveEventMedel alloc] init];
        ...
        case 0:
        {
            if (section == 2 ) {
                if (self.jsplDownone == nil ) {
                    ...
                    BIMHSJFoldHeaderView *headerView = [tableView dequeueReusableHeaderFooterViewWithIdentifier:@"header"];
                    if (!headerView) {
                        headerView = [[BIMHSJFoldHeaderView alloc] initWithReuseIdentifier:@"header"];
                    }
        ...
        case 1: 
        {
            switch (section) {
                case 0:
                {
                    ...
                    BIMBisaiTJHeaderView *view = [[BIMBisaiTJHeaderView alloc] initWithFrame:CGRectMake(0, 0, Width, 170)];
    ...
    switch (_currentIndex) {
        case 0:
        {
            if (indexPath.section == 0 ) {
                if (indexPath.row == 0) {
                    BIMJishiFirstCell *jiShiCell = [tableView dequeueReusableCellWithIdentifier:jiShiFirstCellID forIndexPath:indexPath];
                    jiShiCell.jiShiPLFirstModel = [BIMJiShiPLFirstModel new];
                    jiShiCell.selectionStyle = UITableViewCellSelectionStyleNone;
                    return jiShiCell;
                }else {
                    BIMJiShiPeiLvCell *jiShiPeiLvCell = [tableView cellForRowAtIndexPath:indexPath];
                    if (!jiShiPeiLvCell) {
                        jiShiPeiLvCell = [[BIMJiShiPeiLvCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:jiShiPeilvDetailCellID];
                    }
                ...
                BIMJSPLDownCell *jSPLDownC = [tableView cellForRowAtIndexPath:indexPath];
                if (!jSPLDownC) {
                    jSPLDownC = [[BIMJSPLDownCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:jSPLDownCelliD];
                }
                ...
                BIMJSPLDownCell *jSPLDownTwo = [tableView cellForRowAtIndexPath:indexPath];
                if (!jSPLDownTwo) {
                    jSPLDownTwo = [[BIMJSPLDownCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:jSPLDownCelliD];
                }
        ...
        case 1:
        {
            switch (indexPath.section) {
                ...
                case 2:
                {
                    BIMBisaiTongjiCell *cell = [tableView dequeueReusableCellWithIdentifier:cellBisaiTongji];
                    if (!cell) {
                        cell = [[BIMBisaiTongjiCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellBisaiTongji];
                    }
                    BIMLiveEventMedel*model=_event[indexPath.row];
                ...
                case 1:
                {
                    BIMBisaiTongjiCellTwo *cell = [tableView dequeueReusableCellWithIdentifier:cellBisaiTongjiTwo];
                    if (!cell) {
                        cell = [[BIMBisaiTongjiCellTwo alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellBisaiTongjiTwo];
                    }
        ...
        case 2:
        {
            switch (indexPath.section) {
                case 0:
                {
                    BIMDoubleBattlecell *battleCell = [tableView dequeueReusableCellWithIdentifier:doubleBattleCell forIndexPath:indexPath];
                    if (_upArr.count > 0) {
    ...
    switch (_currentIndex) {
        case 0:
        {
            if (indexPath.section == 0 || indexPath.section == 1) {
                BIMPeiLvDetailVC *peilvDDT = [[BIMPeiLvDetailVC alloc]init];
...
- (void)loadJiShiPLData {
    ...
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:nil PathUrlL:lsURl Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            self.defaultTitle = @"";
            NSLog(@"即使指数请求结果=%@",responseOrignal);
            _jsplDownone = [BIMJSPLDownMode entityFromDictionary:[[[responseOrignal objectForKey:@"data"] objectForKey:@"CornerOdds"] objectForKey:@"Rf_Odds"]];
            _jsplDownTwo = [BIMJSPLDownTwoModel entityFromDictionary:[[[responseOrignal objectForKey:@"data"] objectForKey:@"CornerOdds"] objectForKey:@"Dx_Odds"]];
...
- (void)loadDoubleBatData {
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:nil PathUrlL:[NSString stringWithFormat:@"%@%@%ld.json",APPDELEGATE.url_jsonHeader,@"/jsbf/live/newbacklive_",(long)self.model.mid] Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
        NSLog(@"双方阵容=%@",responseOrignal);
        if ([responseOrignal objectForKey:@"up"]) {
            _upArr = [[NSMutableArray alloc] initWithArray:[BIMBattleModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"up"]]];
            if (_upArr.count>0) {
                BIMBattleModel*model=[BIMBattleModel new];
                model.homenumber=@"";
                model.guestnumber=@"";
        ...
        if ([responseOrignal objectForKey:@"back"]) {
            _backArr = [[NSMutableArray alloc] initWithArray:[BIMBattleModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"back"]]];
            if (_backArr.count>0) {
                BIMBattleModel*model=[BIMBattleModel new];
                model.homenumber=@"";
                model.guestnumber=@"";
...
- (void)loadDataZhiBo{
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:nil PathUrlL:[NSString stringWithFormat:@"%@%@%ld.json",APPDELEGATE.url_jsonHeader,@"/jsbf/live/newlive_",(long)self.model.mid] Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
        NSLog(@"统计返回=%@",responseOrignal);
        _event = [[NSMutableArray alloc] initWithArray:[BIMLiveEventMedel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"event"]]];
...
NSLog(@"返回的数据333=%ld",_event.count);
        if ([responseOrignal objectForKey:@"tech"]) {
            _tech = [[NSArray alloc] initWithArray:[BIMTechModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"tech"]]];
        }
        if ([responseOrignal objectForKey:@"techtwo"]) {
            _techtwo = [[NSArray alloc] initWithArray:[BIMTechtwoModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"techtwo"]]];
        }
        if ([responseOrignal objectForKey:@"homestatis"]) {
            _homestatis =[[NSArray alloc] initWithArray:[BIMliveLineupModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"homestatis"]]];
        }
        if ([responseOrignal objectForKey:@"gueststatis"]) {
            _gueststatis = [[NSArray alloc] initWithArray:[BIMliveLineupModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"gueststatis"]]];
        }
                [self reloadData];
    } Failure:^(NSError *error, NSString *errorDict, id responseOrignal) {
    }];
...
- (void)webSocket:(SRWebSocket *)webSocket didReceiveMessage:(id)message {
    ...
    if (ddd.length>0) {
        NSMutableArray *arrEvent = [[NSMutableArray alloc] initWithArray:[BIMLiveEventMedel arrayOfEntitiesFromArray:[TongJiDic objectForKey:@"event"]]];
        BIMLiveEventMedel *start = [[BIMLiveEventMedel alloc] init];
        start.headerOrFooter = 1;
        BIMLiveEventMedel *end = [[BIMLiveEventMedel alloc] init];
        end.headerOrFooter = 2;
        ...
        if ([TongJiDic objectForKey:@"tech"]) {
            _tech = [[NSArray alloc] initWithArray:[BIMTechModel arrayOfEntitiesFromArray:[TongJiDic objectForKey:@"tech"]]];
        }
        if ([TongJiDic objectForKey:@"techtwo"]) {
            _techtwo = [[NSArray alloc] initWithArray:[BIMTechtwoModel arrayOfEntitiesFromArray:[TongJiDic objectForKey:@"techtwo"]]];
        }
        if ([TongJiDic objectForKey:@"homestatis"]) {
            _homestatis =[[NSArray alloc] initWithArray:[BIMliveLineupModel arrayOfEntitiesFromArray:[TongJiDic objectForKey:@"homestatis"]]];
        }
        if ([TongJiDic objectForKey:@"gueststatis"]) {
            _gueststatis = [[NSArray alloc] initWithArray:[BIMliveLineupModel arrayOfEntitiesFromArray:[TongJiDic objectForKey:@"gueststatis"]]];
        }
    }else{
        [self loadDataZhiBo];
    }
