...
@interface BIMSaishiSelecterdVC ()<ViewPagerDelegate,ViewPagerDataSource,TitleIndexViewDelegate,SelectedAllVCDelegate,SelectedJincaiVCDelegate,SelectedChupanVCDelegate,NavViewDelegate>
...
@end
@implementation BIMSaishiSelecterdVC
    ...
    switch (index) {
        case 0:
        {
            if (self.saishiBtn.isSelected) {
                BIMSelectedAllVC *SelectedV  = [[BIMSelectedAllVC alloc] init];
                ...
                BIMSelectedJincaiVC *SelectedV  = [[BIMSelectedJincaiVC alloc] init];
                SelectedV.type = _type;
                SelectedV.timeline =  self.timeline;
...
- (void)confirmSelectedChupanWithData:(NSArray *)arrSaveData
{
    ...
    if (_type == typeSaishiSelecterdVCBifen) {
        for (int i = 0; i<_arrBifenData.count; i++) {
            BIMJSbifenModel *jsmodel = [_arrBifenData objectAtIndex:i];
            BIMJSbifenModel *sendJs = [[BIMJSbifenModel alloc] init];
            ...
            for (int m = 0; m<jsmodel.data.count; m++) {
                ...
                for (int j = 0; j<arrSaveData.count; j++) {
                    BIMBIfenSelectedSaishiModel *modelSave = [arrSaveData objectAtIndex:j];
    ...
    {
        for (int i = 0; i<_arrBifenData.count; i++) {
            BIMLiveScoreModel *model = [_arrBifenData objectAtIndex:i];
            NSArray *arrletgoal = [model.letgoal componentsSeparatedByString:@","];
            NSArray *arrtotal = [model.total componentsSeparatedByString:@","];
            NSArray *arrstandard = [model.standard componentsSeparatedByString:@","];
            for (int j = 0; j<arrSaveData.count; j++) {
...
- (void)confirmSelectedJincaiWithData:(NSArray *)arrSaveData
{
    ...
    if (_type == typeSaishiSelecterdVCBifen) {
        ...
        [arrSaveData enumerateObjectsUsingBlock:^(BIMBIfenSelectedSaishiModel  * obj, NSUInteger idx, BOOL * _Nonnull stop) {
