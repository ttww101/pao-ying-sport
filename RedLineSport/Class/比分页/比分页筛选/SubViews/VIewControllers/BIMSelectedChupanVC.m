...
@interface BIMSelectedChupanVC ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,AllSelectedViewDelegate>
...
@end
@implementation BIMSelectedChupanVC
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)seperateData
{
    ...
    for (int i = 0; i<_arrData.count; i++) {
        BIMBIfenSelectedSaishiModel *model = [_arrData objectAtIndex:i];
        if ([[NSUserDefaults standardUserDefaults] boolForKey:@"loadedBifenData"]) {
            ...
            for (int j = 0; j<arrsaveSaishi.count; j++) {
                BIMBIfenSelectedSaishiModel *Savemodel = [arrsaveSaishi objectAtIndex:j];
    ...
    if (index == 0) {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(confirmSelectedChupanWithData:)]) {
            ...
            for (int i = 0 ;i<arrMemory.count; i++) {
                BIMBIfenSelectedSaishiModel *model = [arrMemory objectAtIndex:i];
                if (model.isSelected) {
...
- (UICollectionView *)collectionView
{
    if (!_collectionView) {
        ...
        [self.collectionView registerClass:[BIMSelectedCCell class] forCellWithReuseIdentifier:cellCollectioneSlectedChupan];
...
{
    BIMSelectedCCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:cellCollectioneSlectedChupan forIndexPath:indexPath];
    if (!cell) {
        NSLog(@"无法创建cell");
    }
    else{
    ...
    BIMBIfenSelectedSaishiModel *model = [arr objectAtIndex:indexPath.row];
