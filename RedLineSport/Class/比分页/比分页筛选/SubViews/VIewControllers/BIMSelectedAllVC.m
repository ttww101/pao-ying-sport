#import "BIMSelectedAllVC.h"
#import "BIMAllSelectedView.h"
#import "BIMDSCollectionViewIndex.h"
#import "BIMBIfenSelectedSaishiModel.h"
#import "BIMSelectedCCell.h"
...
@interface BIMSelectedAllVC ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,AllSelectedViewDelegate,DSCollectionViewIndexDelegate>
@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, strong) BIMAllSelectedView *allselectedV;
@property (nonatomic, strong) NSArray *arrViewIndex;
@property (nonatomic, strong) BIMDSCollectionViewIndex *ViewIndex;
...
@end
@implementation BIMSelectedAllVC
...
//    NSString *path = [NSString stringWithFormat:@"http://120.55.30.173:8809%@",url_bifen_filterAll];
    ...
    [[BIMDCHttpRequest shareInstance]sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:path Start:^(id requestOrignal) {
...
- (void)checkoutAllSelected {
    ...
    for (NSInteger i = 0; i < self.dataList.count; i ++) {
        ...
        for (NSInteger j = 0 ; j < model.dataList.count; j ++) {
            BIMBIfenSelectedSaishiModel *bifenmodel = model.dataList[j];
...
- (BIMAllSelectedView *)allselectedV
{
    if (!_allselectedV) {
        _allselectedV = [[BIMAllSelectedView alloc] initWithFrame:CGRectMake(0, Height - 44, Width, 44)];
...
- (void)didSelectedAtBtnIndex:(NSInteger)index whtherSelected:(BOOL)selected {
     if (index == 0) {
        [self.dataList enumerateObjectsUsingBlock:^(FilterData * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            [obj.dataList enumerateObjectsUsingBlock:^(BIMBIfenSelectedSaishiModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
                obj.isSelected = true;
            }];
        }];
        [self.collectionView reloadData];
...
- (UICollectionView *)collectionView
{
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[BIMSelectedCCell class] forCellWithReuseIdentifier:CollectionCellSelectedAllVC];
...
{
    BIMSelectedCCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:CollectionCellSelectedAllVC forIndexPath:indexPath];
    if (!cell) {
        NSLog(@"无法创建cell");
    }
    else{
    ...
    BIMBIfenSelectedSaishiModel *model = [dataModel.dataList objectAtIndex:indexPath.row];
...
- (BIMDSCollectionViewIndex *)ViewIndex
{
    if (!_ViewIndex) {
        _ViewIndex = [[BIMDSCollectionViewIndex alloc] initWithFrame:CGRectMake(Width - 30, APPDELEGATE.customTabbar.height_myNavigationBar + 44, 30, Height - APPDELEGATE.customTabbar.height_myNavigationBar - 44 - _allselectedV.height)];
