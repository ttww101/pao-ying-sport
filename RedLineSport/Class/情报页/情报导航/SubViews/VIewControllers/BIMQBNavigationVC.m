...
#import "BIMQBNavigatCell.h"
#import "BIMNavigationModel.h"
#import "BIMFenxiPageVC.h"
#import "BIMLiveScoreModel.h"
@interface BIMQBNavigationVC ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
...
@end
@implementation BIMQBNavigationVC
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [[BIMUMStatisticsMgr sharedInstance] viewStaticsBeginWithMarkStr:@"BIMQBNavigationVC"];
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [[BIMUMStatisticsMgr sharedInstance] viewStaticsEndWithMarkStr:@"BIMQBNavigationVC"];
}
...
- (void)loadData
{
    ...
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,@"/mobile/news/fullList"] Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            ...
            _arrAllDate = [[NSMutableArray alloc] initWithArray:[BIMNavigationModel arrayOfEntitiesFromArray: [responseOrignal objectForKey:@"data"]]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    BIMQBNavigatCell *cell = [tableView dequeueReusableCellWithIdentifier:aCell];
    if (!cell) {
        cell = [[BIMQBNavigatCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:aCell];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    BIMNavigationModel *model = _arrAllDate[indexPath.row];
