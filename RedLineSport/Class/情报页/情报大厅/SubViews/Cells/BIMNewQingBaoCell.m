...
@end
@implementation BIMNewQingBaoCell
...
- (UILabel *)labContent{
    if (!_labContent) {
        ...
        _labContent.attributedText = [BIMMethods setTextStyleWithString:_labContent.text WithLineSpace:5.5 WithHeaderIndent:Width - 30];
    }
    return _labContent;
}
- (UILabel *)labHome{
...
- (void)setModel:(BIMQingBaoFPTwoModel *)model{
    _model = model;
    self.labTime.text = [NSString stringWithFormat:@"发布于 %@",[BIMMethods getDateByStyle:@"MM月dd日 HH:mm" withDate:[NSDate dateWithTimeIntervalSince1970:[model.createtime doubleValue]/1000]]];
    self.labTitle.numberOfLines = 2;
    self.labTitle.text = [NSString stringWithFormat:@"%@",model.title];
    self.labTitle.attributedText = [BIMMethods setTextStyleWithString:self.labTitle.text WithLineSpace:5 WithHeaderIndent:Width - 30];
    ...
    self.labFabuTime.text = [NSString stringWithFormat:@"%@",[BIMMethods getDateByStyle:dateStyleFormatterMdHm withDate:[NSDate dateWithTimeIntervalSince1970:[model.mtime doubleValue]/1000]]];
    self.labLeg.text = model.league;
    self.labLeg.textColor = [BIMMethods getColor:model.leagueColor];
    self.labContent.attributedText = [BIMMethods setTextStyleWithString:self.labContent.text WithLineSpace:5.5 WithHeaderIndent:Width - 30];
    switch (model.mark) {
        case 1:{
