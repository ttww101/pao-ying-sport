...
#import "BIMNewQingBaoCell.h"
#import "BIMFenxiPageVC.h"
#import "BIMLiveScoreModel.h"
#import "BIMQBNavigationVC.h"
...
@interface BIMNewQingBaoViewController ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate, GBMenuButtonDelegate>
...
@end
@implementation BIMNewQingBaoViewController
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [[BIMUMStatisticsMgr sharedInstance] viewStaticsBeginWithMarkStr:@"BIMNewQingBaoViewController"];
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    [[BIMUMStatisticsMgr sharedInstance] viewStaticsEndWithMarkStr:@"BIMNewQingBaoViewController"];
}
- (void)dealloc {
...
- (void)loadData
{
    ...
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,@"/news/v1.2/infolist"] Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            if (_limitStart == 0) {
               _arrAllDate = [[NSMutableArray alloc] initWithArray:[BIMQingBaoFPTwoModel arrayOfEntitiesFromArray: [responseOrignal objectForKey:@"data"]]];
            }else{
                NSArray *arr = [[NSMutableArray alloc] initWithArray:[BIMQingBaoFPTwoModel arrayOfEntitiesFromArray: [responseOrignal objectForKey:@"data"]]];
                if (arr.count == 0) {
...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        [_tableView registerClass:[BIMNewQingBaoCell class] forCellReuseIdentifier:aCell];
...
- (void)navViewTouchAnIndex:(NSInteger)index
{
    if (index == 1) {
        ...
        BIMQBNavigationVC *navigat = [[BIMQBNavigationVC alloc] init];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    BIMQingBaoFPTwoModel *model = _arrAllDate[indexPath.section];
    CGFloat heiContent = [BIMMethods getTextHeightStationWidth:[NSString stringWithFormat:@"       %@",model.content] anWidthTxtt:Width - 30 anfont:14 andLineSpace:5.5 andHeaderIndent:0];
    if (heiContent > 84) {
        heiContent = 83.32;
    }
    CGFloat heiTitle = [BIMMethods getTextHeightStationWidth:model.title anWidthTxtt:Width - 30 anfont:20 andLineSpace:5 andHeaderIndent:0];
    return   heiContent + heiTitle + 74;
    return 0;
}
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    BIMNewQingBaoCell *cell = [tableView dequeueReusableCellWithIdentifier:aCell forIndexPath:indexPath];
    if (!cell) {
        cell = [[BIMNewQingBaoCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:aCell];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    BIMQingBaoFPTwoModel *_matchModel = [_arrAllDate objectAtIndex:indexPath.section];
