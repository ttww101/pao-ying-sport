...
#import "BIMJiaoYiNewCell.h"
#import "BIMLiveScoreModel.h"
#import "BIMFenxiPageVC.h"
@interface BIMJiaoYiNewTabelView()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
...
@end
@implementation BIMJiaoYiNewTabelView
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    BIMJiaoYiNewCell *cell = [tableView dequeueReusableCellWithIdentifier:acell];
    if (!cell) {
        cell = [[BIMJiaoYiNewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:acell];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    BIMJiaoYiModel *model = _dataArr[indexPath.row];
    if (!_isToFenxi == YES) {
        _isToFenxi = YES;
    }else{
        return;
...
- (void)getDataJiaoyi
{
    ...
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_JiaoYiList] Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] intValue] == 200) {
            self.dataArr = [NSArray arrayWithArray:[BIMJiaoYiModel arrayOfEntitiesFromArray:responseOrignal[@"data"]]] ;
