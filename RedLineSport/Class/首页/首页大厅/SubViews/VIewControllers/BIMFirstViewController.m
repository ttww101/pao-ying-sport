#import "BIMFirstViewController.h"
#import "BIMFocusModel.h"
#import "BIMNewslistModel.h"
#import "BIMFirstPInfoListModel.h"
#import "BIMUserlistModel.h"
#import "BIMRollInfoModel.h"
#import "BIMTuijiandatingModel.h"
#import "BIMWebDetailViewController.h"
#import "BIMLoadHtmlStringVC.h"
#import "BIMTuijianDatingCell.h"
#import "BIMNewRecommandVC.h"
#import "BIMfirstHotInfoCycleView.h"
#import "BIMFirstPUserlistView.h"
#import "BIMTuijianDetailVC.h"
...
#import "BIMVierticalScrollView.h"
#import "BIMFenxiPageVC.h"
#import "BIMTuijianDTViewController.h"
#import "BIMLaunchView.h"
#import "BIMGuideView.h"
#import "BIMTodayHotSpotsCell.h"
#import "BIMRedDanCycleView.h"
#import "BIMTuijianDatingCell.h"
#import "BIMTuijiandatingModel.h"
#import "BIMRedDanModel.h"
#import "BIMMostModel.h"
#import "BIMLivingModel.h"
#import "BIMDataModelView.h"
#import "BIMTodayHotSpotsTwoCell.h"
#import "BIMModelPredictionViewController.h"
#import "BIMToolKitViewController.h"
#import "BIMLiveViewController.h"
#import "BIMToolWebViewController.h"
@interface BIMFirstViewController ()<UITableViewDelegate,UITableViewDataSource,firstHotInfoCycleViewDelegate,FirstPUserlistViewDelegate,VierticalScrollViewDelegate,SDCycleScrollViewDelegate,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate,DataModelViewDelegate>
@property (nonatomic, strong) BIMBasicTableView *tableView;
@property (nonatomic, strong) SDCycleScrollView *cycleView;
@property (nonatomic, strong) BIMDataModelView *viewInfo;
...
@property (nonatomic, strong) BIMNewslistModel *column;
...
@property (nonatomic, strong)BIMfirstHotInfoCycleView *firstPg;
...
@end
@implementation BIMFirstViewController
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [MobClick beginLogPageView:@"BIMFirstViewController"];
...
- (void)timerChange
{
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:[BIMHttpString getCommenParemeter] PathUrlL:[NSString stringWithFormat:@"%@%@%@",APPDELEGATE.url_jsonHeader,@"/jsbf",url_jsbf_change] Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
        NSArray *arrLiving = [BIMLivingModel arrayOfEntitiesFromArray:responseOrignal];
...
- (void)viewDidLoad {
    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"showGuideVC"]) {
        BIMGuideView *guideV = [[BIMGuideView alloc] initWithFrame:CGRectMake(0, 0, Width, Height)];
    ...
    if (dataFirstPageDataLocal) {
        ...
        if ([dictFirstPageDataLocal isKindOfClass:[NSDictionary class]]) {
            _arrFocus = [[NSMutableArray alloc] initWithArray:[BIMFocusModel arrayOfEntitiesFromArray:[[dictFirstPageDataLocal objectForKey:@"data"] objectForKey:@"focusNews"]]];
            if (![[[dictFirstPageDataLocal objectForKey:@"data"] objectForKey:@"hotMatches"] isKindOfClass:[NSNull class]]) {
                _arrInfolist = [[NSArray alloc] initWithArray:[BIMFirstPInfoListModel arrayOfEntitiesFromArray:[[dictFirstPageDataLocal objectForKey:@"data"] objectForKey:@"hotMatches"]]];
            }
            _arrToHotzMost = [[NSArray alloc] initWithArray:[BIMMostModel arrayOfEntitiesFromArray:[[dictFirstPageDataLocal objectForKey:@"data"] objectForKey:@"most"]]];
            _arrUserListBigName = [[NSArray alloc] initWithArray:[BIMUserlistModel arrayOfEntitiesFromArray:[[dictFirstPageDataLocal objectForKey:@"data"] objectForKey:@"userlist"][@"bigNameList"]]];
            _arrUserListRedList = [[NSArray alloc] initWithArray:[BIMUserlistModel arrayOfEntitiesFromArray:[[dictFirstPageDataLocal objectForKey:@"data"] objectForKey:@"userlist"][@"redList"]]];
            _arrUserListWeekList = [[NSArray alloc] initWithArray:[BIMUserlistModel arrayOfEntitiesFromArray:[[dictFirstPageDataLocal objectForKey:@"data"] objectForKey:@"userlist"][@"weekList"]]];
            _arrRedDan = [[NSArray alloc] initWithArray:[BIMRedDanModel arrayOfEntitiesFromArray:[[dictFirstPageDataLocal objectForKey:@"data"] objectForKey:@"redNews"]]];
            _arrRollInfo = [[NSArray alloc] initWithArray:[BIMRollInfoModel arrayOfEntitiesFromArray:[[dictFirstPageDataLocal objectForKey:@"data"] objectForKey:@"rollInfo"]]];
            _arrNewslist = [NSArray arrayWithArray:[BIMNewslistModel arrayOfEntitiesFromArray:[[dictFirstPageDataLocal objectForKey:@"data"] objectForKey:@"newslist"]]];
            _hotRecommed = [NSArray arrayWithArray:[BIMTuijiandatingModel arrayOfEntitiesFromArray:[[dictFirstPageDataLocal objectForKey:@"data"] objectForKey:@"essenceRecommend"]]];
        }
    }
...
{
    if (!_tableView) {
        self.tableView = [[BIMBasicTableView alloc] initWithFrame:CGRectMake(0, 0, Width, Height - 49)];
        self.tableView.separatorStyle = UITableViewCellSeparatorStyleNone;
        [self.tableView registerClass:[BIMTuijianDatingCell class] forCellReuseIdentifier:cellFirstPageTuijian];
...
- (void)headerRefresh
{
    ...
    if (activityArray.count == 0) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            [[BIMDCHttpRequest shareInstance]sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_liveQuiz] Start:^(id requestOrignal) {
            } End:^(id responseOrignal) {
            } Success:^(id responseResult, id responseOrignal) {
...
- (BIMfirstHotInfoCycleView *)firstPg{
    if (!_firstPg) {
        _firstPg = [[BIMfirstHotInfoCycleView alloc] initWithFrame:CGRectMake(0, 0, Width,200)];
    }
    return _firstPg;
}
    ...
    switch (indexPath.section) {
        ...
        case 1:
        {
            ...
            if (indexPath.row == 0) {
                ...
                BIMTodayHotSpotsCell *cellNull = [tableView dequeueReusableCellWithIdentifier:cell];
                if (!cellNull) {
                    cellNull = [[BIMTodayHotSpotsCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cell];
                }
                ...
                BIMTodayHotSpotsTwoCell *cellNull = [tableView dequeueReusableCellWithIdentifier:cell];
                if (!cellNull) {
                    cellNull = [[BIMTodayHotSpotsTwoCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cell];
                }
                cellNull.selectedBackgroundView = [[UIView alloc] initWithFrame:cellNull.frame];
                cellNull.selectedBackgroundView.backgroundColor = colorF5;
                cellNull.row = indexPath.row;
        ...
        case 3://推荐名人
        {
            ...
            cellNull.selectionStyle = UITableViewCellSelectionStyleNone;
            BIMFirstPUserlistView *subVOne = [[BIMFirstPUserlistView alloc] initWithFrame:CGRectMake(0, 0, Width, 220)];
            switch (_btnUerBtnIndex) {
                case 1:{
        ...
        case 4://红单播报
        {
            ...
            BIMRedDanCycleView *redView = [[BIMRedDanCycleView alloc] initWithFrame:CGRectMake(0, 0, Width, 180)];
        ...
        case 5:
        {
            ...
            BIMTuijianDatingCell *cellNull = [tableView dequeueReusableCellWithIdentifier:cell];
            if (!cellNull) {
                cellNull = [[BIMTuijianDatingCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cell];
            }
            cellNull.selectedBackgroundView = [[UIView alloc] initWithFrame:cellNull.frame];
            cellNull.selectedBackgroundView.backgroundColor = colorF5;
    ...
    switch (section) {
        case 0:
        {
            ...
            for (int i = 0; i<_arrFocus.count; i++) {
                BIMFocusModel *model = [_arrFocus objectAtIndex:i];
                if (model.pic == nil) {
                    model.pic = @"";
                }
                if (model.title == nil) {
            ...
            _cycleView.imageUBIMtringsGroup = arrpics;
            ...
            if (_arrRollInfo.count >0) {
                ...
                for (int i = 0; i<_arrRollInfo.count; i++) {
                    BIMRollInfoModel *roll =  [_arrRollInfo objectAtIndex:i];
                    if (roll.on_index_title == nil || [roll.on_index_title isEqualToString:@""]) {
                        roll.on_index_title = @"";
                    }
                    [arrTitle addObject:roll.on_index_title];
                }
                BIMVierticalScrollView *scroView = [BIMVierticalScrollView initWithTitleArray:arrTitle AndFrame:CGRectMake(viewShu.right + 10, 0, Width - viewShu.right - 15, 44)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 5) {
        BIMTuijianDetailVC *infoVC = [[BIMTuijianDetailVC alloc] init];
        infoVC.hidesBottomBarWhenPushed = YES;
        BIMTuijiandatingModel *model = _hotRecommed[indexPath.row];
    ...
    }else if(indexPath.section == 6)
    {
        ...
        switch (indexPath.row) {
            case 0:{
                BIMMostModel *modelMost = _arrToHotzMost[indexPath.row];
...
{
    BIMFocusModel *fmodel = [_arrFocus objectAtIndex:index];
    switch (fmodel.linkType) {
        ...
        case 1:
        {
            BIMWebModel *webModel = [[BIMWebModel alloc]init];
            webModel.title = fmodel.title;
            webModel.webUrl = fmodel.url2;
            BIMToolWebViewController *control = [[BIMToolWebViewController alloc]init];
        ...
        case 4:
        {
            BIMTuijianDetailVC *tuijianDT = [[BIMTuijianDetailVC alloc] init];
        ...
        case 6:
        {
            BIMLoadHtmlStringVC *htmlVC = [[BIMLoadHtmlStringVC alloc] init];
...
{
    BIMRollInfoModel *roll = [_arrRollInfo objectAtIndex:button.tag -1];
    [self toFenxiWithMatchId:[NSString stringWithFormat:@"%ld",(long)roll.match_id] toPageindex:2 toItemIndex:0];
}
- (BIMDataModelView *)viewInfo
{
    if (!_viewInfo) {
        _viewInfo = [[BIMDataModelView alloc] initWithFrame:CGRectMake(0, 0, Width, 100)];
...
- (void)didSelectedDataModelViewIndex:(NSInteger)index;
{
    switch (index) {
        ...
        case 1:
        {
            BIMLiveViewController *control = [[BIMLiveViewController alloc]init];
            [self.navigationController pushViewController:control animated:YES];
        }
            break;
        case 0:
        ...
        case 3:
        {
            BIMToolKitViewController *control = [[BIMToolKitViewController alloc]init];
            [self.navigationController pushViewController:control animated:YES];
        }
            break;
        default:
...
- (void)loadData
{
    ...
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_firstIndex] Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            ...
            _arrFocus = [[NSMutableArray alloc] initWithArray:[BIMFocusModel arrayOfEntitiesFromArray:[[responseOrignal objectForKey:@"data"] objectForKey:@"focusNews"]]];
            if (![[[responseOrignal objectForKey:@"data"] objectForKey:@"hotMatches"] isKindOfClass:[NSNull class]]) {
                NSArray *arrinfo = [[NSArray alloc] initWithArray:[BIMFirstPInfoListModel arrayOfEntitiesFromArray:[[responseOrignal objectForKey:@"data"] objectForKey:@"hotMatches"]]];
                for (int i = 0; i<arrinfo.count; i++) {
                    BIMFirstPInfoListModel *model = [arrinfo objectAtIndex:i];
                    for (int j = 0; j<_arrInfolist.count; j++) {
                        BIMFirstPInfoListModel *mModel = [_arrInfolist objectAtIndex:j];
            ...
            _arrUserListBigName = [[NSArray alloc] initWithArray:[BIMUserlistModel arrayOfEntitiesFromArray:[[responseOrignal objectForKey:@"data"] objectForKey:@"userlist"][@"bigNameList"]]];
            _arrUserListRedList = [[NSArray alloc] initWithArray:[BIMUserlistModel arrayOfEntitiesFromArray:[[responseOrignal objectForKey:@"data"] objectForKey:@"userlist"][@"redList"]]];
            _arrUserListWeekList = [[NSArray alloc] initWithArray:[BIMUserlistModel arrayOfEntitiesFromArray:[[responseOrignal objectForKey:@"data"] objectForKey:@"userlist"][@"weekList"]]];
            _arrRollInfo = [[NSArray alloc] initWithArray:[BIMRollInfoModel arrayOfEntitiesFromArray:[[responseOrignal objectForKey:@"data"] objectForKey:@"rollInfo"]]];
            _arrRedDan = [[NSArray alloc] initWithArray:[BIMRedDanModel arrayOfEntitiesFromArray:[[responseOrignal objectForKey:@"data"] objectForKey:@"redNews"]]];
            _arrNewslist = [NSArray arrayWithArray:[BIMNewslistModel arrayOfEntitiesFromArray:[[responseOrignal objectForKey:@"data"] objectForKey:@"newslist"]]];
            _arrToHotzMost = [[NSArray alloc] initWithArray:[BIMMostModel arrayOfEntitiesFromArray:[[responseOrignal objectForKey:@"data"] objectForKey:@"most"]]];
            _hotRecommed = [NSArray arrayWithArray:[BIMTuijiandatingModel arrayOfEntitiesFromArray:[[responseOrignal objectForKey:@"data"] objectForKey:@"essenceRecommend"]]];
...
- (void)liveQuziAction:(UITapGestureRecognizer *)tap {
    ...
    if ([targetCalss isKindOfClass:NSClassFromString(@"BIMLiveQuizViewController")]) {
        BIMWebModel *model = [[BIMWebModel alloc]init];
        ...
        BIMLiveQuizViewController *controller = [[BIMLiveQuizViewController alloc]init];
        controller.model = model;
        [self.navigationController pushViewController:controller animated:YES];
    } else {
        BIMToolWebViewController *target =[[targetCalss alloc] init];
        BIMWebModel *model = [[BIMWebModel alloc]init];
        NSDictionary *pDic = self.activityDic[@"v"];
        model.title = PARAM_IS_NIL_ERROR(pDic[@"title"]);
        model.webUrl = PARAM_IS_NIL_ERROR(pDic[@"url"]);
