...
@interface BIMTodayHotSpotsTwoCell()
...
@end
@implementation BIMTodayHotSpotsTwoCell
...
-(void)setModel:(BIMMostModel *)model{
    _model = model;
    self.labLeague.text = @"";
    self.labContent.text = model.mark;
    if (!isNUll( self.labContent.text)) {
        self.labContent.attributedText = [BIMMethods setTextStyleWithString:self.labContent.text WithLineSpace:5.5 WithHeaderIndent:200];
    }
    switch (self.row) {
        case 0:{
        }
        ...
        case 1:{
            ...
            if (!isNUll(self.labRedNum.text) ) {
                self.labRedNum.attributedText = [BIMMethods withContent:self.labRedNum.text WithColorText:@"%" textColor:redcolor strFont:font17];
                if (model.type == 3) {
