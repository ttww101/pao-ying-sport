...
@interface BIMfirstHotInfoCycleView()<UICollectionViewDataSource, UICollectionViewDelegate>
@property (nonatomic, weak) UICollectionView *mainView; 
@property (nonatomic, weak) UICollectionViewFlowLayout *flowLayout;
@property (nonatomic, strong) UIPageControl *pagecontrol;
@end
@implementation BIMfirstHotInfoCycleView
...
- (void)setupMainView
{
    ...
    [mainView registerClass:[BIMFirstQBView class] forCellWithReuseIdentifier:@"FirstQBViewcell"];
...
{
    BIMFirstQBView *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"FirstQBViewcell" forIndexPath:indexPath];
...
- (void)setUpData:(NSArray *)arr{
    for (int i = 0; i < arr.count; i++) {
        BIMLivingModel *living = [arr objectAtIndex:i];
        for (int j = 0; j < _arrData.count; j ++)
        {
            BIMFirstPInfoListModel *model = _arrData[j];
