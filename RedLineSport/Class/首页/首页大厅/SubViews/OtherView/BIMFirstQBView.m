#import "BIMFirstQBView.h"
#import "BIMFenxiPageVC.h"
@interface BIMFirstQBView()
...
- (void)setModel:(BIMFirstPInfoListModel *)model{
    _model = model;
    self.labLeague.text = model.league;
    self.labTime.text = [BIMMethods getDateByStyle:dateStyleFormatterMdHm withDate:[NSDate dateWithTimeIntervalSince1970:model.matchtime/1000]];
    ...
    [self.labContent setAttributedText:[BIMMethods setTextStyleWithString:model.matchintro WithLineSpace:6 WithHeaderIndent:0]];
    if (model.info > 0) {
    ...
    if (_model.matchstate == -1) {
        ...
        self.labMiao.text = [BIMMethods getTextByMatchState:_model.matchstate];
        self.labScore.text = @"VS";
        self.labScore.textColor = color33;
        self.labMiao.textColor = redcolor;
    }
