...
@interface BIMTongbeiTongjiCell()
...
@end
@implementation BIMTongbeiTongjiCell
- (void)awakeFromNib {
    [super awakeFromNib];
}
...
- (void)setModel:(BIMTongPeiTJModel *)model
{
    _model = model;
    ...
    _labLeague.textColor = [BIMMethods getColor:_model.sclassColor];
    ...
    _labRate.textColor = [BIMMethods getColor:_model.resultColor];
    _labRateTitle.textColor = [BIMMethods getColor:_model.resultColor];
    _labRound.text = [NSString stringWithFormat:@"共%ld场",_model.num];
    _labRound.textColor =color66;
    _labRate.attributedText = [BIMMethods withContent:_labRate.text WithColorText:@"%" textColor:[BIMMethods getColor:_model.resultColor] strFont:font12];
}
- (UIView *)basicView
{
    if (!_basicView) {
