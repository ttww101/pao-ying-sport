...
#import "BIMTongbeiTongjiCell.h"
#import "BIMTongPeiTongjiTable.h"
#import "BIMTongPeiDetailVC.h"
@interface BIMTongPeiTongjiTable()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
@property (nonatomic, strong) NSArray *arrData;
@property (nonatomic, assign) NSInteger type;
@property (nonatomic, assign) NSInteger currentIndex;
@property (nonatomic, retain)NSArray *arrTitle;
@end
@implementation BIMTongPeiTongjiTable
- (instancetype)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        [self registerClass:[BIMTongbeiTongjiCell class] forCellReuseIdentifier:cellTongPeiTongjiTable];
        self.delegate =self;
...
{
    BIMTongbeiTongjiCell *cell = [tableView dequeueReusableCellWithIdentifier:cellTongPeiTongjiTable];
    if (!cell) {
        cell = [[BIMTongbeiTongjiCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellTongPeiTongjiTable];
    }
...
{
    BIMTongPeiDetailVC *tongPDT = [[BIMTongPeiDetailVC alloc] init];
    BIMTongPeiTJModel *model;
    if (_arrData.count > 0) {
...
{
    NSMutableDictionary *patameter = [NSMutableDictionary dictionaryWithDictionary:[BIMHttpString getCommenParemeter]];
    NSString *mtype = @"";
    ...
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:patameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_sameOdd_index] Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            _arrData = [NSArray arrayWithArray:[BIMTongPeiTJModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"data"] ]];
