...
#import "BIMTongPeiDetailVC.h"
#import "BIMTongpeiDetailCell.h"
#import "BIMTongpeiDTResultView.h"
#import "BIMTongpeiDTTitileView.h"
#import "BIMTongPeiSwitch.h"
#import "BIMTitleIndexView.h"
#import "BIMTongpeiDTModel.h"
#import "BIMTongPeiPeiLvDTVC.h"
@interface BIMTongPeiDetailVC ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate,TongPeiSwitchDelegate,TitleIndexViewDelegate>
@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, strong) BIMTitleIndexView *titleView;
@property (nonatomic, strong) BIMTongpeiDTModel *spfModel;
@property (nonatomic, strong) BIMTongpeiDTModel *yaModel;
@property (nonatomic, strong) BIMTongpeiDTModel *dxModel;
@property (nonatomic, strong) BIMTongpeiDTModel *currentModel;
@property (nonatomic, assign) NSInteger currentIndex;
@property (nonatomic, strong) UIImageView *imageRedAngle;
@end
@implementation BIMTongPeiDetailVC
...
- (void)viewDidLoad {
    ...
    _titleView = [[BIMTitleIndexView alloc] initWithFrame:CGRectMake(0, Height - 44, Width, 44)];
...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        [_tableView registerClass:[BIMTongpeiDetailCell class] forCellReuseIdentifier:cellTongPeiDetailVC];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    BIMTongpeiDTResultView *resultView = [[BIMTongpeiDTResultView alloc] initWithFrame:CGRectMake(0, 0, Width, 35)];
    ...
    BIMTongPeiSwitch *btnSwitch = [[BIMTongPeiSwitch alloc] init];
    ...
    if (_currentIndex == 0) {
        if (_currentModel.all.matchs.count == 0) {
            ...
            BIMTongpeiDTTitileView *titleView = [[BIMTongpeiDTTitileView alloc] initWithFrame:CGRectMake(0, 0, Width, 30)];
...
{
    BIMTongpeiDetailCell *cell = [tableView dequeueReusableCellWithIdentifier:cellTongPeiDetailVC];
    if (!cell) {
        cell = [[BIMTongpeiDetailCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellTongPeiDetailVC];
    }
...
{
    [[BIMDependetNetMethods sharedInstance] requestSameOdd_detailWithscheduleId:[NSString stringWithFormat:@"%ld",_scheduleId] WithsclassId:[NSString stringWithFormat:@"%ld",_sclassId] Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] intValue]== 200) {
            _spfModel = [BIMTongpeiDTModel entityFromDictionary:[[responseOrignal objectForKey:@"data"] objectForKey:@"spf"]];
            _dxModel = [BIMTongpeiDTModel entityFromDictionary:[[responseOrignal objectForKey:@"data"] objectForKey:@"dx"]];
            _yaModel = [BIMTongpeiDTModel entityFromDictionary:[[responseOrignal objectForKey:@"data"] objectForKey:@"ya"]];
            switch (_pelvIndex) {
                case 1:
                    _currentModel = _yaModel;
                    break;
