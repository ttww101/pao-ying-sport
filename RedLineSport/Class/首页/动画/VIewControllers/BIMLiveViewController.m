...
@interface BIMLiveViewController () <TYTabPagerControllerDataSource, TYTabPagerControllerDelegate, TYTabPagerBarDelegate>
@property (nonatomic, strong) NSArray *newsPageInfos;
@end
@implementation BIMLiveViewController
...
- (void)loadData{
    ...
    [[BIMDCHttpRequest shareInstance]sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:@"http://api.live.gunqiu.com:88/radar?action=getDays" Start:^(id requestOrignal) {
...
- (CGFloat)pagerTabBar:(TYTabPagerBar *)pagerTabBar widthForItemAtIndex:(NSInteger)index {
    ...
    CGFloat width = [BIMMethods widthForString:newsPageInfo[@"name"] fontSize:17.f andHeight:0];
    return width;
}
...
- (UIViewController *)tabPagerController:(TYTabPagerController *)tabPagerController controllerForIndex:(NSInteger)index prefetching:(BOOL)prefetching {
    NSDictionary *newsPageInfo = _newsPageInfos[index];
    BIMLiveListViewController *control = [[BIMLiveListViewController alloc]init];
