...
@interface BIMLiveQuizWithdrawalTableViewCell ()
@property (nonatomic, strong) UIView *lineView;
@property (nonatomic, strong) UILabel *timeLabel;
@property (nonatomic, strong) UILabel *vsLabel;
@property (nonatomic, strong) UILabel *addMoneyLabel;
@end
@implementation BIMLiveQuizWithdrawalTableViewCell
static CGFloat cell_Height = 70.f;
static NSString *identifier = @"listCell";
+ (BIMLiveQuizWithdrawalTableViewCell *)cellForTableView:(UITableView *)tableView {
    BIMLiveQuizWithdrawalTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (cell == nil) {
        cell = [[BIMLiveQuizWithdrawalTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
...
- (void)refreshContentData:(id)model {
    BIMWithdrawalModel *data = (BIMWithdrawalModel *)model;
    self.timeLabel.text = [BIMMethods formatMMDDWithStamp:data.created];
    self.vsLabel.text = data.item_name;
    self.addMoneyLabel.text = [NSString stringWithFormat:@"+%@元", [BIMMethods amountFormater:data.amount]];
}
