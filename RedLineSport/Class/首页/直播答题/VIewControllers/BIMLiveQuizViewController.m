...
#import "BIMLiveQuizAlertView.h"
#import "BIMAppManger.h"
#import "BIMJSModel.h"
@interface BIMLiveQuizViewController () <UIWebViewDelegate, UIGestureRecognizerDelegate>
@property (nonatomic, strong) UIWebView *webView;
@property (nonatomic , strong) WebViewJavascriptBridge* bridge;
@property (nonatomic , copy) GQJSResponseCallback callBack;
@property (nonatomic , assign) BOOL showLoding;
@property (nonatomic , strong) BIMAppManger *manger;
@property (nonatomic , strong) UIView *toastView;
@end
@implementation BIMLiveQuizViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    self.showLoding = YES;
    [self configUI];
...
- (void)loadBradgeHandler {
    __weak BIMLiveQuizViewController *weakSelf = self;
...
- (void)dialog:(id)parameter {
    ...
    NSDictionary *dic = [NSJSONSerialization JSONObjectWithData:jsonData
                                                        options:NSJSONReadingMutableContainers
                                                          error:&err];
    [BIMLiveQuizAlertView showPaymentInfo:dic animations:YES selectOption:^(id selectAction) {
        if (selectAction) {
            self.callBack(@"1");
            self.showLoding = false;
            [self.webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:@"about:blank"]]];
...
- (BIMAppManger *)manger {
    if (_manger == nil) {
        _manger = [[BIMAppManger alloc]init];
    }
    return _manger;
}
@end
