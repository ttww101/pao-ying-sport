...
#import "BIMPanwangCell.h"
#import "BIMPanwangZhishuTable.h"
#define cellPanwangCell @"cellPanwangCell"
@interface BIMPanwangZhishuTable()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
...
@end
@implementation BIMPanwangZhishuTable
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        [self registerClass:[BIMPanwangCell class] forCellReuseIdentifier:cellPanwangCell];
        self.delegate =self;
...
{
    BIMPanwangCell *cell = [tableView dequeueReusableCellWithIdentifier:cellPanwangCell];
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    if (!cell) {
        cell = [[BIMPanwangCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellPanwangCell];
    }
    cell.rankNum = indexPath.row + 1;
    BIMPanWangModel *model = self.arrData[indexPath.row];
    cell.model = model;
...
- (void)loadDataPanwangWithType:(loadDataType)loadType
{
    ...
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,@"/recordodds"] Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] intValue] == 200) {
            ...
            NSArray *arr = [BIMPanWangModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"data"]];
            if (arr.count == 0) {
