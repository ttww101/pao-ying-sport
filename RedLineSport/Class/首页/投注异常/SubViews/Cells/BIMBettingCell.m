...
@property (nonatomic, strong)UILabel *labTBIM;
...
- (UILabel *)labTBIM{
    if (!_labTBIM) {
        _labTBIM = [[UILabel alloc] init];
        _labTBIM.text = @"74%";
        _labTBIM.textColor = color33;
        _labTBIM.font = font11;
        _labTBIM.textAlignment = NSTextAlignmentCenter;
    }
    return _labTBIM;
}
- (UILabel *)labWuCha{
...
-(void)setModel:(BIMTouZhuModel *)model{
    _model = model;
    ...
    self.labTBIM.text = model.deal[@"profit"];
...
- (void)setMas{
    ...
    [self.labTBIM mas_makeConstraints:^(MASConstraintMaker *make) {
    ...
    [self.labgaiLv mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.mas_equalTo(self.labTBIM.mas_left);
        make.centerY.mas_equalTo(self.labTBIM.mas_centerY);
