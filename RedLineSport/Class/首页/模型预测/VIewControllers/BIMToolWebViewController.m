...
#import "BIMAppleIAPService.h"
#import <UShareUI/UShareUI.h>
#import "BIMCouponListViewController.h"
#import "BIMLiveQuizWithDrawalViewController.h"
#import "XHPayKit.h"
#import "ArchiveFile.h"
#import "BIMAppManger.h"
#import "BIMNavImageView.h"
#import "BIMWebView.h"
#import "BIMLiveQuizViewController.h"
#import "BIMLotteryWebViewController.h"
#import "BIMCommentsView.h"
#import "BIMCommentsViewController.h"
#import "BIMInputViewController.h"
#import "BIMReplyViewController.h"
#import "GeneralFloatingView.h"
#import "GBPopMenuButtonView.h"
#import "BIMNewQingBaoViewController.h"
#import "WebNavDelegate.h"

@interface BIMToolWebViewController () <UIWebViewDelegate, GQWebViewDelegate, WKUIDelegate,WKNavigationDelegate, CommentsViewDelegate, GeneralFloatingViewDelegate, UIWebViewDelegate, GBMenuButtonDelegate>
...
@property (nonatomic , strong) BIMCommentsView *commentsView;
...
#define wxpay @"wx"
#define alipay @"ali"
@implementation BIMToolWebViewController
...
-(void)menuButtonSelectedAtIdex:(NSInteger)index {
    ...
    if (_menuButtonView.itemsImages.count == 5) {
        ...
        if(![BIMMethods login]) {
            ...
            BIMToolWebViewController *target =[[BIMToolWebViewController alloc] init];
        ...
        BIMFabuTuijianSelectedItemVC *control= [[BIMFabuTuijianSelectedItemVC alloc]init];
        ...
        if (_wkWeb) {
            ...
            BIMNewQingBaoViewController *vc = [BIMNewQingBaoViewController new];
...
//        [_wkWeb loadRequest:request];
        ...
        if ([self.model.webUrl isEqualToString:homePageUrl]) {
            ...
            BIMDCTabBarController *rootController = (BIMDCTabBarController*)[[(BIMAppDelegate*)
...
- (void)commentViewDidSelectReply:(BIMCommentsView *)commentView {
    if (self.commentsDic) {
        NSDictionary *dic = self.commentsDic[@"comment"];
        BIMInputViewController *control = [[BIMInputViewController alloc]init];
...
- (void)loadBradgeHandler {
    __weak BIMToolWebViewController *weakSelf = self;
    BIMAppManger *manger = [[BIMAppManger alloc]init];
    ...
    if (self.useWkWeb) {
        bridge = [manger WK_RegisterJSTool:self.wkWeb hannle:^(id data, GQJSResponseCallback responseCallback) {
            ...
            BIMJSModel *model = (BIMJSModel *)data;
...
- (void)loadData {
    ...
    if (self.urlPath != nil) {
        ...
        NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url cachePolicy:NSURLRequestReloadRevalidatingCacheData timeoutInterval:15];
        [request setValue:PARAM_IS_NIL_ERROR([BIMMethods getTokenModel].token) forHTTPHeaderField:@"token"];
        if (self.useWkWeb) {
        ...
        if ([_model.parameter isKindOfClass:[NSDictionary class]]) {
            ...
            if (leftArray.count > 0) {
                ...
                for (NSInteger i = 0; i < leftArray.count; i ++) {
                    ...
                    BIMNavImageView *imageV = [[BIMNavImageView alloc]init];
...
- (void)webShare:(id)data {
    if ([data isKindOfClass:NSClassFromString(@"NSDictionary")]) {
        ...
        [UMSocialUIManager showShareMenuViewInWindowWithPlatformSelectionBlock:^(UMSocialPlatformType platformType, NSDictionary *userInfo) {
            ...
            [[UMSocialManager defaultManager] shareToPlatform:platformType messageObject:messageObject currentViewController:[BIMMethods help_getCurrentVC] completion:^(id data, NSError *error) {
                if (error) {
...
- (void)openNative:(id)data {
    if ([data isKindOfClass:[NSDictionary class]]) {
        ...
        className = [NSString stringWithFormat:@"BIM%@",className];
        if ([className isEqualToString:@"BIMstartChat"]) {
            if(![BIMMethods login]) {
                [BIMMethods toLogin];
            } else {
                BIMToolWebViewController *target =[[BIMToolWebViewController alloc] init];
        ...
        if ([className isEqualToString:@"BIMGQMineViewController"]) {
            className = @"BIMMineViewController";
        }
        if ([className isEqualToString:@"BIMFenxiPageVC"]) {
            [self pushToMatchDetail:dataDic];
            return;
        }
        if ([className isEqualToString:@"BIMBifenViewController"]) {
            [self.tabBarController setSelectedIndex:1];
            return;
        }
        if ([className isEqualToString:@"BIMNewQingBaoViewController"]) {
            [self.tabBarController setSelectedIndex:2];
            return;
        }
        if ([className isEqualToString:@"BIMLiveQuizViewController"]) {
            BIMWebModel *model = [[BIMWebModel alloc]init];
        ...
        if ([className isEqualToString:@"BIMLotteryWebViewController"]) {
            BIMWebModel *model = [[BIMWebModel alloc]init];
            NSDictionary *vDic = dataDic[@"v"];
            model.title = PARAM_IS_NIL_ERROR(vDic[@"title"]);
            model.webUrl = PARAM_IS_NIL_ERROR(vDic[@"url"]);
            model.hideNavigationBar = [vDic[@"nav_hidden"] integerValue];
            BIMLotteryWebViewController *controller = [[BIMLotteryWebViewController alloc]init];
            controller.model = model;
            [self.navigationController pushViewController:controller animated:YES];
            return;
        }
        ...
        if ([className isEqualToString:@"BIMFabuTuijianSelectedItemVC"]) {
        ...
        if ([className isEqualToString:@"BIMSearchViewController"]) {
...
- (void)replyAction {
    ...
    BIMReplyViewController *control = [[BIMReplyViewController alloc]init];
...
- (void)tableBarAction:(UITapGestureRecognizer *)tap {
    BIMNavImageView *imageView = (BIMNavImageView *)tap.view;
...
- (void)payAction:(id)data {
    ...
    __weak BIMToolWebViewController *weakSelf = self;
    [BIMSelectPayMentView showPaymentInfo:[NSString stringWithFormat:@"￥%@",PARAM_IS_NIL_ERROR(data[@"amount"])] options:array  animations:YES selectOption:^(payMentType type) {
        switch (type) {
...
- (void)tencentBuyWithData:(NSDictionary *)data {
    ...
    [[BIMDCHttpRequest shareInstance]sendGetRequestByMethod:@"post" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_modelPay] Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
...
- (void)alibuyWithData:(NSDictionary *)data {
    ...
    [parameter setObject:@"IOS" forKey:@"resource"];
    [parameter setObject:PARAM_IS_NIL_ERROR(data[@"scheduleId"]) forKey:@"matchId"];
    [[BIMDCHttpRequest shareInstance]sendGetRequestByMethod:@"post" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_modelPay_ali] Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
        [BIMLodingAnimateView dissMissLoadingView];
        NSDictionary *resultDic = (NSDictionary *)responseOrignal;
...
- (void)appleBuyWithData:(NSDictionary *)data {
    ...
    [[BIMAppleIAPService sharedInstance]purchase:@{@"product_id":PARAM_IS_NIL_ERROR(productId), @"orderID":ordeId, @"amount":@(amount)} resultBlock:^(NSString *message, NSError *error) {
        [hud hide:YES];
        if (error) {
...
- (void)couponBuyWithData:(NSDictionary *)data {
    ...
    [parameter setObject:@"IOS" forKey:@"resource"];
    [parameter setObject:PARAM_IS_NIL_ERROR(data[@"scheduleId"]) forKey:@"matchId"];
    [[BIMDCHttpRequest shareInstance]sendGetRequestByMethod:@"post" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_modelPay_coupon] Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
        [BIMLodingAnimateView dissMissLoadingView];
        NSDictionary *resultDic = (NSDictionary *)responseOrignal;
...
- (BIMCommentsView *)commentsView {
    if (_commentsView == nil) {
        _commentsView = [[BIMCommentsView alloc]init];
