...
#import "BIMBaolengDetailVC.h"
#import "BIMBaolengDTHeaderView.h"
#import "BIMBaolengSwitch.h"
#import "BIMBaolengDTcell.h"
#import "BIMBaolengDTTitleView.h"
#import "BIMBaolengDetailModel.h"
@interface BIMBaolengDetailVC ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate,BaolengSwitchDelegate>
@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, strong) BIMBaolengDetailModel*baolengModel;
@property (nonatomic, strong) BIMBaolengDetailModel*leaguecoldinfo;
@property (nonatomic, strong) BIMBaolengDetailModel*historycoldinfo;
@property (nonatomic, strong) BIMBaolengDetailModel*allcoldinfo;
@property (nonatomic, assign) NSInteger currentIndex;
@end
@implementation BIMBaolengDetailVC
...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        [_tableView registerClass:[BIMBaolengDTcell class] forCellReuseIdentifier:cellBaolengDetailVC];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    BIMBaolengDTHeaderView *baolengDT = [[BIMBaolengDTHeaderView alloc] initWithFrame:CGRectMake(0, 0, Width, 45)];
    ...
    BIMBaolengSwitch *btnSwitch = [[BIMBaolengSwitch alloc] init];
    ...
    if (_baolengModel.list.count != 0) {
        BIMBaolengDTTitleView *titleV = [[BIMBaolengDTTitleView alloc] initWithFrame:CGRectMake(0, 0, Width, 30)];
...
{
    BIMBaolengDTcell *cell = [tableView dequeueReusableCellWithIdentifier:cellBaolengDetailVC];
    if (!cell) {
        cell = [[BIMBaolengDTcell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellBaolengDetailVC];
    }
...
{
    [[BIMDependetNetMethods sharedInstance] requestSurpriseWithType:[NSString stringWithFormat:@"%ld",_idId] Start:^(id requestOrignal) {
        ...
        if ([responseOrignal objectForKey:@"code"]) {
            _leaguecoldinfo = [BIMBaolengDetailModel entityFromDictionary:[[responseOrignal objectForKey:@"data"] objectForKey:@"leaguecoldinfo"]];
            _historycoldinfo = [BIMBaolengDetailModel entityFromDictionary:[[responseOrignal objectForKey:@"data"] objectForKey:@"historycoldinfo"]];
            _allcoldinfo = [BIMBaolengDetailModel entityFromDictionary:[[responseOrignal objectForKey:@"data"] objectForKey:@"allcoldinfo"]];
            _baolengModel = _allcoldinfo;
            [self setNavView];
            [self.tableView reloadData];
        }
