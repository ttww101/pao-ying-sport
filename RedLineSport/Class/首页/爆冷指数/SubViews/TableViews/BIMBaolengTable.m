...
#import "BIMBaolengZhishuCell.h"
#import "BIMBaolengDetailVC.h"
#import "BIMBaolengZishuModel.h"
#import "BIMBaolengTable.h"
@interface BIMBaolengTable()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
@property (nonatomic, retain)NSArray *arrData;
@property (nonatomic, assign) NSInteger currentIndex;
@end
@implementation BIMBaolengTable
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        [self registerClass:[BIMBaolengZhishuCell class] forCellReuseIdentifier:cellBaolengZhishuVC];
...
{
    BIMBaolengZhishuCell *cell = [tableView dequeueReusableCellWithIdentifier:cellBaolengZhishuVC];
    if (!cell) {
        cell = [[BIMBaolengZhishuCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellBaolengZhishuVC];
    }
...
{
    BIMBaolengDetailVC *baolengDT = [[BIMBaolengDetailVC alloc] init];
    BIMBaolengZishuModel* model = [_arrData objectAtIndex:indexPath.row];
...
- (void)loadBaolengData
{
    ...
    [[BIMDependetNetMethods sharedInstance] requeSurprisestatisWithType:mtype Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
        if ([[responseOrignal objectForKey:@"code"] intValue] == 200) {
            self.defaultTitle = @"暂无数据";
            _arrData = [NSArray arrayWithArray:[BIMBaolengZishuModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"data"]]];
            [self reloadData];
        }else{
