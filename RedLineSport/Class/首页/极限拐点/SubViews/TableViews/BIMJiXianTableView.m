...
#import "BIMJiXianCell.h"
#import "BIMRecord_OneModel.h"
#import "BIMLiveScoreModel.h"
#import "BIMFenxiPageVC.h"
@interface BIMJiXianTableView()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
...
@end
@implementation BIMJiXianTableView
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    BIMJiXianCell *cell = [tableView dequeueReusableCellWithIdentifier:acell];
    if (!cell) {
        cell = [[BIMJiXianCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:acell];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    BIMRecord_OneModel *model = self.arrData[indexPath.row];
    if (!_isToFenxi == YES) {
        _isToFenxi = YES;
    }else{
        return;
...
- (void)loadDataJixian
{
    ...
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_recordodds] Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
        if ([[responseOrignal objectForKey:@"code"] intValue] == 200) {
            _arrData = [NSArray arrayWithArray:[BIMRecord_OneModel arrayOfEntitiesFromArray:responseOrignal[@"data"]]];
