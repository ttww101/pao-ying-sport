...
#import "BIMPlycSelectedView.h"
#import "BIMYpasCell.h"
#import "BIMYpasTwoCell.h"
#define  cellYpasCell @"cellYpasCell"
#define  cellYpasTwoCell @"cellYpasTwoCell"
@interface BIMYapanZhoushouVC ()<TitleIndexViewDelegate,PlycSelectedViewdelegate,UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
@property (nonatomic, strong) BIMTitleIndexView *titleView;
@property (nonatomic, strong) BIMPlycSelectedView *selectedView;
...
@end
@implementation BIMYapanZhoushouVC
...
- (void)viewDidLoad {
    ...
    _titleView = [[BIMTitleIndexView alloc] initWithFrame:CGRectMake(0, APPDELEGATE.customTabbar.height_myNavigationBar, Width, 48)];
    ...
    _selectedView = [[BIMPlycSelectedView alloc] initWithFrame:CGRectMake(0, 0, Width, Height)];
...
- (void)loadPeilvYichangVCDataWithType:(loadDataType)type
{
    ...
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,_typeOdd == 0? url_letgoalAbnormalindex :url_totalAbnormalindex] Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] integerValue] == 200) {
            ...
            NSArray *arr = [NSArray arrayWithArray:[BIMPlycModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"data"]]];
...
{
    if (!_tableView) {
        _tableView = [[BIMBasicTableView alloc] initWithFrame:CGRectMake(0, APPDELEGATE.customTabbar.height_myNavigationBar + 48 + 10, Width, Height - APPDELEGATE.customTabbar.height_myNavigationBar - 48 - 10) style:UITableViewStylePlain];
        _tableView.separatorStyle = UITableViewCellSeparatorStyleNone;
        _tableView.showsVerticalScrollIndicator = NO;
        [_tableView registerClass:[UITableViewCell class] forCellReuseIdentifier:@"cell"];
        [_tableView registerClass:[BIMYpasCell class] forCellReuseIdentifier:cellYpasCell];
        [_tableView registerClass:[BIMYpasTwoCell class] forCellReuseIdentifier:cellYpasTwoCell];
...
{
    if (_currentIndex == 0) {
        BIMPlycModel *mod = [_arrdata objectAtIndex:indexPath.row];
...
{
    if (_currentIndex == 0) {
        BIMYpasCell *cell = [tableView dequeueReusableCellWithIdentifier:cellYpasCell];
        if (!cell) {
            cell = [[BIMYpasCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellYpasCell];
        }
        ...
        BIMYpasTwoCell *cell = [tableView dequeueReusableCellWithIdentifier:cellYpasTwoCell];
        if (!cell) {
            cell = [[BIMYpasTwoCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellYpasTwoCell];
        }
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        cell.model = [_arrdata objectAtIndex:indexPath.row];
        return cell;
