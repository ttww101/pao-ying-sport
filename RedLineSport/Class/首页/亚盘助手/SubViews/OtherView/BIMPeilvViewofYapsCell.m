...
@interface BIMPeilvViewofYapsCell()
...
@end
@implementation BIMPeilvViewofYapsCell
- (void)setModel:(BIMPlycModel *)model
{
    ...
    if (!isNUll(_model.finalTimeCn)&& ![_model.finalTimeCn isEqualToString:@"全部"]) {
        ...
        _labTimeJP.text = [BIMMethods timeToNowWith:_model.finalTime];
    }
    _labCPTitle.text = @"初赔:";
    _labCP1.text  = _model.firstWin;
    _labCP2.text  = _model.firstDraw;
    _labCP3.text  = _model.firstLose;
    _labTimeCP.text = [BIMMethods timeToNowWith:_model.firstTime];
    ...
    for (int i = 0; i<_model.panProcess.count; i++) {
        BIMPanProcessModel *process = [_model.panProcess objectAtIndex:i];
       ...
       UILabel *labTime = [[UILabel alloc] initWithFrame:CGRectMake(lab3.right + 8.5, 0, 50, 10)];
        ...
        labTime.font = font7;
        labTime.text = [BIMMethods timeToNowWith:process.modifyTime];
