...
@interface BIMInfoTableViewCell ()
...
@end
@implementation BIMInfoTableViewCell
...
+ (BIMInfoTableViewCell *)cellForTableView:(UITableView *)tableView {
    BIMInfoTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (cell == nil) {
        cell = [[BIMInfoTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
...
- (void)setModel:(InfoGroupModel *)model {
    ...
    self.dataLabel.text = [BIMMethods compareCurrentTime:_model.date];
