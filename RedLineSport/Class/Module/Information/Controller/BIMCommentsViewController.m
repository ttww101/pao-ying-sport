...
@interface BIMCommentsViewController ()  <UITableViewDataSource, UITableViewDelegate, InfoTableViewCellDelegate>
...
@end
@implementation BIMCommentsViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    [self configUI];
    [self setupHeader];
...
- (void)loadData {
    _limitStart = 0;
    _limitNum = 20;
    __weak BIMCommentsViewController *weakSelf = self;
...
- (void)loadMore {
    ...
    [self.viewModel fetchRecommendedReviewsWithParameter:parameter callBack:^(BOOL isSucess, id response) {
        ...
        if (isSucess) {
            BIMInfoModel *model = [BIMInfoModel yy_modelWithDictionary:(NSDictionary *)response];
...
- (void)tableViewCell:(BIMInfoTableViewCell *)cell moreComments:(id)moreComments {
    BIMCommentsDetailViewController *control = [[BIMCommentsDetailViewController alloc]init];
