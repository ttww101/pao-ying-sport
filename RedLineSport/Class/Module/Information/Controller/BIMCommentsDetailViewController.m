...
#import "BIMCommentsDetailViewModel.h"
#import "BIMInfoTableViewCell.h"
#import <YYModel/YYModel.h>
#import "BIMHeaderTableViewCell.h"
#import "BIMCommentsViewController.h"
#import "BIMDeatilHeaderView.h"
#import "BIMInputViewController.h"
@interface BIMCommentsDetailViewController () <UITableViewDataSource, UITableViewDelegate, InfoTableViewCellDelegate>
@property (nonatomic , strong) BIMCommentsDetailViewModel *viewModel;
@property (nonatomic , strong) UITableView *tableView;
@property (nonatomic , strong) BIMInfoModel *model;
@property (nonatomic , strong) BIMDeatilHeaderView *headerView;
@property (nonatomic , strong) UIButton *replyBtn;
@property (nonatomic, assign) NSInteger limitStart;
@property (nonatomic, assign) NSInteger limitNum;
@end
@implementation BIMCommentsDetailViewController
- (void)viewDidLoad {
    [super viewDidLoad];
    [self configUI];
    [self setupHeader];
...
- (void)loadData {
    _limitStart = 0;
    _limitNum = 20;
    __weak BIMCommentsDetailViewController *weakSelf = self;
    ...
    [self.viewModel fetchCommentsListWithParams:parameter callBack:^(BOOL isSucess, id response) {
        ...
        if (isSucess) {
            ...
            weakSelf.model = [BIMInfoModel yy_modelWithDictionary:dic];
...
- (void)loadMore {
    ...
    [self.viewModel fetchCommentsListWithParams:parameter callBack:^(BOOL isSucess, id response) {
        ...
        if (isSucess) {
            NSDictionary *responseDic = (NSDictionary *)response[@"data"];
            NSDictionary *dic = @{@"data":responseDic[@"son"]};
            BIMInfoModel *model = [BIMInfoModel yy_modelWithDictionary:dic];
...
- (BIMCommentsDetailViewModel *)viewModel {
    if (_viewModel == nil) {
        _viewModel = [[BIMCommentsDetailViewModel alloc]init];
    }
    return _viewModel;
}
...
- (BIMDeatilHeaderView *)headerView {
    if (_headerView == nil) {
        _headerView = [[BIMDeatilHeaderView alloc]initWithFrame:CGRectMake(0, 0, Width, 132)];
    }
    return _headerView;
}
- (UIButton *)replyBtn {
