...
@interface BIMPeilvViewOfTuijianCell()
...
@end
@implementation BIMPeilvViewOfTuijianCell
...
- (void)setModel:(BIMTuijiandatingModel *)model
{
    ...
    if (_model.see) {
        if (_model.spf.count>0) {
            ...
            _labPeilv.attributedText = [BIMMethods withContent:_labPeilv.text WithColorText:@"@" textColor:color33 strFont:font14];
        ...
        _labchoice.text = [NSString stringWithFormat:@" %ld 连红币",_model.amount==3800?30:_model.amount/100];
    }
}
- (UIView *)basicView
{
