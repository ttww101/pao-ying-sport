...
#import "BIMUserViewOfTuijianCell.h"
@interface BIMUserViewOfTuijianCell()
...
@end
@implementation BIMUserViewOfTuijianCell
- (id)init
{
    self = [super init];
    if (self) {
...
- (void)setValueWithUserTitle:(NSString *)title
                    ...
                    WithModel:(BIMTuijiandatingModel *)model
{
    if (!_addedAutoLayout) {
    ...
    [titls enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        BIMMedalsModel *medals = (BIMMedalsModel*)obj;
        switch (idx) {
            case 0:
            {
    ...
    if (remark.count == 0) {
        ...
        BIMUsermarkModel *usermark = [remark firstObject];
        if (usermark.type == 1) {
        ...
        for (int i = 0; i<remark.count; i++) {
            BIMUsermarkModel *usermark = [remark objectAtIndex:i];
            if (usermark.type == 1) {
