...
#import "BIMTuijianDatingTable.h"
#import "BIMTuijianSelectedItemTitleView.h"
#import "BIMTuijianSelectedItemView.h"
#import "BIMNewRecommandVC.h"
#define CellTuijianDating @"CellTuijianDating"
@interface BIMTuijianDatingTable()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate,TuijianSelectedItemTitleViewDelegate,TuijianSelectedItemViewDelegate>
@property (nonatomic,strong) BIMTuijianSelectedItemView *selectedView;
@property (nonatomic,strong) BIMTuijianSelectedItemTitleView *titleView;
...
@end
@implementation BIMTuijianDatingTable
...
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style playType:(NSInteger)type;
{
    ...
    if (self) {
        ...
        self.defaultTitle = @"";
        [self registerClass:[BIMTuijianDatingCell class] forCellReuseIdentifier:CellTuijianDating];
        self.delegate =self;
...
- (BIMTuijianSelectedItemView *)selectedView
{
    if (!_selectedView) {
        _selectedView = [[BIMTuijianSelectedItemView alloc] initWithFrame:CGRectMake(0 ,0, Width, Height - APPDELEGATE.customTabbar.height_myNavigationBar)];
...
{
    BIMNewRecommandVC *recmdLVC = [[BIMNewRecommandVC alloc] init];
    if (btn.tag == 0) {
...
- (BIMTuijianSelectedItemTitleView *)titleView
{
    if (!_titleView) {
        _titleView = [[BIMTuijianSelectedItemTitleView alloc] initWithFrame:CGRectMake(0, 0, Width, 44)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BIMTuijianDatingCell *cell = [tableView dequeueReusableCellWithIdentifier:CellTuijianDating];
    if (!cell) {
        cell = [[BIMTuijianDatingCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellTuijianDating];
    }
...
- (void)loadDataByloadDataType:(loadDataType)loadType;
{
    ...
    NSMutableDictionary *parmater = [NSMutableDictionary dictionaryWithDictionary:[BIMHttpString getCommenParemeter]];
    ...
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parmater PathUrlL:[[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_newlistrecommend] copy] Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            ...
            NSArray *arr = [BIMTuijiandatingModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"data"]];
            self.hideFooter = NO;
            if (arr.count == 0) {
...
- (void)loadShaixuanItems
{
    ...
    for (int i = 0; i<arrUrl_jsonRecommand.count; i++) {
        [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:nil PathUrlL:[NSString stringWithFormat:@"%@%@%@",APPDELEGATE.url_jsonHeader,@"/recommend/",[arrUrl_jsonRecommand objectAtIndex:i]] Start:^(id requestOrignal) {
        } End:^(id responseOrignal) {
        } Success:^(id responseResult, id responseOrignal) {
        } Failure:^(NSError *error, NSString *errorDict, id responseOrignal) {
        }];
