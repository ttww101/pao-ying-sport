...
@interface BIMDan_StringGuanVC ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
...
@property (nonatomic, strong)BIMDan_StringMatchsModel *selectModel;
@property (nonatomic, strong)UIButton *saiXuanBtn;
@end
@implementation BIMDan_StringGuanVC
...
- (void)getdataWithtype:(loadDataType)type{
    if (type == loadDataFirst || type == loadDataHeaderRefesh) {
        ...
        if (_type == 0 && _arrData.count != 0 && _arrDataTime.count != 0) {
            ...
            for (int i = 0; i < _arrData.count; i ++) {
                ...
                BIMDan_StringGuanModel *model = _arrData[i];
                NSMutableArray *arr = [[NSMutableArray alloc] initWithCapacity:0];
                for (int j = 0; j < model.matchs.count; j++) {
                    [arr addObject:@(0)];
                    BIMDan_StringMatchsModel *dan_StringModel = model.matchs[j];
                    BIMLiveScoreModel *liveScoreModel = [[BIMLiveScoreModel alloc] init];
    ...
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:nil PathUrlL:[NSString stringWithFormat:@"%@",self.urlTypeStr] Start:^(id requestOrignal) {
        ...
        if (_type ==0) {
            _arrData = [[NSMutableArray alloc] initWithArray:[BIMDan_StringGuanModel arrayOfEntitiesFromArray:responseOrignal]];
            _arrDataTime = [[NSMutableArray alloc] initWithArray:[BIMDan_StringGuanModel arrayOfEntitiesFromArray:responseOrignal]];
        }else{
            _arrData = [[NSMutableArray alloc] initWithArray:[BIMDan_StringGuanModel arrayOfEntitiesFromArray:responseOrignal]];
            _arrDataLeageue = [[NSMutableArray alloc] initWithArray:[BIMDan_StringGuanModel arrayOfEntitiesFromArray:responseOrignal]];
        }
        self.arrDataAll = [BIMDan_StringGuanModel arrayOfEntitiesFromArray:responseOrignal];
        for (int i = 0; i < _arrData.count; i ++) {
            if (i == 0 && self.type == 0) {
                [_arrSection addObject:@(1)];
            }else{
        ...
        NSString *documentPath = [BIMMethods getDocumentsPath];
...
- (void)saiXuan{
    ...
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:nil PathUrlL:str_url Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
        _arrDataShaiShiAll = [[NSMutableArray alloc] initWithArray:[BIMBIfenSelectedSaishiModel arrayOfEntitiesFromArray:responseOrignal[@"allindex"]]];
        _arrDataShaiShiOdds = [[NSMutableArray alloc] initWithArray:[BIMBIfenSelectedSaishiModel arrayOfEntitiesFromArray:responseOrignal[@"oddsindex"]]];
        _arrDataShaiShiJiCai = [[NSMutableArray alloc] initWithArray:[BIMBIfenSelectedSaishiModel arrayOfEntitiesFromArray:responseOrignal[@"jcindex"]]];
        BIMSaishiSelecterdVC *selectedVC = [[BIMSaishiSelecterdVC alloc] init];
...
- (void)updateByselectedsaishi:(NSNotification *)notification
{
    ...
    if ([typeNum_QJC isEqualToString:@"2"]) {
        for (int i = 0; i <  AlldataArrary.count; i ++) {
            BIMLiveScoreModel *model =  AlldataArrary[i];
...
- (void)shaixuanRQ:(NSArray *)arrChooes{
    ...
    for (int i = 0; i < arrDataT.count; i ++) {
        [_arrSection addObject:@(0)];
        BIMDan_StringGuanModel *model = arrDataT[i];
        NSMutableArray *arr = [[NSMutableArray alloc] initWithCapacity:0];
        NSMutableArray *arrMatch = [[NSMutableArray alloc] initWithCapacity:0];
        for (int j = 0; j < model.matchs.count; j++) {
            BIMDan_StringMatchsModel *matchModel = model.matchs[j];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    if ([_arrSection[section] integerValue]== 1) {
        BIMDan_StringGuanModel *model = _arrData[section];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    BIMDan_StringGuanModel *model = [_arrData objectAtIndex:indexPath.section];
    BIMDan_StringMatchsModel *matchModel = model.matchs[indexPath.row];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    labLeage.textColor = [BIMMethods getColor:matchModel.leagueColor];
    labLeage.font = font11;
    if (_type == 0) {
    }else{
        labLeage.textColor = color66;
        labLeage.text = [NSString stringWithFormat:@"%@",[BIMMethods getDateByStyle:dateStyleFormatterMdHm withDate:[NSDate dateWithTimeIntervalSince1970:[matchModel.matchtime doubleValue]/1000]]];
    ...
    lab.text = [NSString stringWithFormat:@"%@",[BIMMethods getDateByStyle:dateStyleFormatterMdHm withDate:[NSDate dateWithTimeIntervalSince1970:[matchModel.matchtime doubleValue]/1000]]];
...
- (void)cellClick:(id)sender{
    ...
    BIMDan_StringGuanModel *model = [_arrData objectAtIndex:_indexPath.section];
    BIMDan_StringMatchsModel *matchModel = model.matchs[_indexPath.row];
...
- (UIView *)createSectionViewRow:(NSInteger)row{
    BIMDan_StringGuanModel *model = [_arrData objectAtIndex:row];
    ...
    for (int i = 0; i<model.matchs.count; i++) {
        BIMDan_StringMatchsModel *matchModel = model.matchs[i];
        if (matchModel.spf.count == 0 && matchModel.rq.count == 0 && matchModel.dx.count == 0) {
        }else{
            count ++;
        }
    ...
    lableague.attributedText = [BIMMethods withContent:lableague.text contentColor:color66 WithColorText:[NSString stringWithFormat:@"  %d",count] textColor:redcolor];
    ...
    if ([_arrSection[row] integerValue]== 1) {
        img.image  = [BIMMethods image:[UIImage imageNamed:@"hongSanJiao0"] rotation:UIImageOrientationUp];
    }else{
        img.width = 7;
        img.height = 10;
        img.image  = [BIMMethods image:[UIImage imageNamed:@"hongSanJiao0"] rotation:UIImageOrientationLeft];
    }
