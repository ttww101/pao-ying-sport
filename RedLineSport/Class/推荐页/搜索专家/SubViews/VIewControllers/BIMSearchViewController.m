...
#import "BIMSearchVCZhujiaCell.h"
@interface BIMSearchViewController ()<UITableViewDelegate,UITableViewDataSource,UISearchBarDelegate>
...
@end
@implementation BIMSearchViewController
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        ...
        BIMSearchVCZhujiaCell *cell = [tableView dequeueReusableCellWithIdentifier:flag];
        if (!cell) {
            cell = [[[NSBundle mainBundle]loadNibNamed:@"BIMSearchVCZhujiaCell" owner:nil options:nil] firstObject];
        }
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (!_searchBar.isFirstResponder) {
        if (indexPath.section == 0) {
            BIMUserlistModel *user = [_arrZhunjia objectAtIndex:indexPath.row];
            if (user.userid == 1) {
                return;
            }
...
- (void)searchTuijianInfoWithName:(NSString *)nickName
{
    ...
    [[BIMDCHttpRequest shareInstance]sendGetRequestByMethod:@"get" WithParamaters:parmater PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_loginAndRegister] Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            _arrZhunjia = [NSArray arrayWithArray:[BIMUserlistModel  arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"data"]]];
            [_searchBar resignFirstResponder];
            [self.tableView reloadData];
...
{
    NSMutableDictionary *parmater = [NSMutableDictionary dictionaryWithDictionary:[BIMHttpString getCommenParemeter]];
    [[BIMDCHttpRequest shareInstance]sendGetRequestByMethod:@"get" WithParamaters:parmater PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_speciallist_hot] Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
