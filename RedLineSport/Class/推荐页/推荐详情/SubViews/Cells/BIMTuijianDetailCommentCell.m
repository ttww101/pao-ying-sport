...
#import "BIMCommentDetailView.h"
@interface BIMTuijianDetailCommentCell()<CommentDetailViewDelegate>
{
...
@end
@implementation BIMTuijianDetailCommentCell
...
- (void)setModel:(BIMCommentModel *)model
{
    _model = model;
    ...
    _labCreatTime.text = [BIMMethods timeToNowWith:_model.createtime];
    _labComentContent.text = _model.content;
    [_labComentContent setAttributedText:[BIMMethods setTextStyleWithString:_model.content WithLineSpace:3 WithHeaderIndent:0]];
    ...
    if (_model.child.count == 0) {
        ...
        for (int i = 0; i<count; i++) {
            ...
            BIMCommentChildModel *child = [_model.child objectAtIndex:i];
            CGFloat labchildhight = 25 +  [BIMMethods getTextHeightStationWidth:child.content anWidthTxtt:ViewBottomWidth - 5  anfont:13 andLineSpace:3 andHeaderIndent:0];
            BIMCommentDetailView *childView = [[BIMCommentDetailView alloc] initWithFrame:CGRectMake(0, viewbottomHight - 15, ViewBottomWidth, labchildhight)];
...
- (void)showBtn
{
    if (self.viewHud.hidden) {
        for (BIMCommentDetailView *child in _arrCells) {
            [child hideHudView];
        }
...
- (void)touchChildCommentViewWithIdid:(NSInteger)Idid
{
    ...
    for (int i = 0; i<_arrCells.count; i++) {
        BIMCommentChildModel *child = [_model.child objectAtIndex:i];
        if (child.Idid != Idid) {
            BIMCommentDetailView *childView = [_arrCells objectAtIndex:i];
