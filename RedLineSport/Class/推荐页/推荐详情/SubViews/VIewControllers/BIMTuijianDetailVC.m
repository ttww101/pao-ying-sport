...
#import "ArchiveFile.h"
@interface BIMTuijianDetailVC () <UITextViewDelegate>
@property (nonatomic, strong) BIMTuijianDetailTableView *tableView;
...
@end
@implementation BIMTuijianDetailVC
- (void)viewWillAppear:(BOOL)animated {
    [super viewWillAppear:animated];
    [[BIMUMStatisticsMgr sharedInstance] viewStaticsBeginWithMarkStr:@"BIMTuijianDetailVC"];
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    [[BIMUMStatisticsMgr sharedInstance] viewStaticsEndWithMarkStr:@"BIMTuijianDetailVC"];
}
...
- (BIMTuijianDetailTableView *)tableView {
    if (!_tableView) {
        _tableView = [[BIMTuijianDetailTableView alloc] initWithFrame:CGRectMake(0, APPDELEGATE.customTabbar.height_myNavigationBar, Width, _status == 1 ? (Height - APPDELEGATE.customTabbar.height_myNavigationBar - 49) : (Height - APPDELEGATE.customTabbar.height_myNavigationBar - self.bottomView.height)) style:UITableViewStylePlain];
...
- (void)paySuccess {
    ...
    BIMDCHttpRequest *httpRequest = [BIMDCHttpRequest shareInstance];
    if (![BIMMethods login]) {
        httpRequest = [BIMDCHttpRequest guestInstance];
    ...
    [httpRequest sendRequestByMethod:@"post"
        ...
        Success:^(id responseResult, id responseOrignal) {
            ...
            if ([[responseOrignal objectForKey:@"code"] integerValue] == 200 || [msg isEqualToString:@"服务端程序异常，请稍后再试"]) {
                ...
                // reload webPage
                BOOL isWebView = [NSStringFromClass([self.navigationController.viewControllers[0] class]) isEqualToString:@"BIMToolWebViewController"];
                if (isWebView) {
                    BIMToolWebViewController *toolWebViewController = self.navigationController.viewControllers[0];
    ...
    [httpRequest sendGetRequestByMethod:@"get"
        ...
        Success:^(id responseResult, id responseOrignal) {
            if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
                ...
                _model = nil;
                _model = [BIMTuijiandatingModel entityFromDictionary:news];
                NSArray *recoommentArray = [[NSArray alloc] initWithArray:[BIMCommentModel arrayOfEntitiesFromArray:[[responseOrignal objectForKey:@"data"] objectForKey:@"comments"]]];
                ...
                NSArray *arr = [BIMTuijiandatingModel arrayOfEntitiesFromArray:[[responseOrignal objectForKey:@"data"] objectForKey:@"recs"]];
                ...
                if (_typeTuijianDetailHeader == typeTuijianDetailHeaderCellDanchang) {
                    ...
                    if (!_model.see) {
                        ...
                        _labelQiuBi.font = font14;
                        _labelQiuBi.attributedText = [BIMMethods withContent:_labelQiuBi.text WithColorText:[NSString stringWithFormat:@"%ld 连红币", _model.amount == 3800 ? 30 : _model.amount / 100] textColor:redcolor strFont:font18];
                    } else {
                        self.bottomView.hidden = NO;
                        self.payView.hidden = YES;
                        self.tableView.frame = CGRectMake(0, APPDELEGATE.customTabbar.height_myNavigationBar, Width, 0);
...
- (void)sendCommentWithCommentTag:(NSInteger)commentTag withDict:(NSDictionary *)dictP {
    ...
    [BIMLodingAnimateView showLodingView];
    [[BIMDCHttpRequest shareInstance] sendRequestByMethod:@"post"
        WithParamaters:parameter
...
- (void)buyActionWithOption:(NSMutableArray *)dataArray {
    ...
    __weak BIMTuijianDetailVC *weakSelf = self;
    [BIMSelectPayMentView showPaymentInfo:[NSString stringWithFormat:@"￥%@", PARAM_IS_NIL_ERROR(@"18")]
                                  options:array
                               animations:YES
    ...
    [httpRequest sendRequestByMethod:@"post"
        ...
        Success:^(id responseResult, id responseOrignal) {
            ...
            if (dic) {
                ...
                NSInteger amount = [BIMMethods amountWithProductId:productID];
                amount = amount * 100;
                [[BIMAppleIAPService sharedInstance] purchase:@{ @"product_id" : productID,
                                                                 @"orderID" : _orderId,
                                                                 @"amount" : @(amount) }
