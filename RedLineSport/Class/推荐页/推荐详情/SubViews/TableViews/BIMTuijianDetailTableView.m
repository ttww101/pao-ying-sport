#import "BIMTuijianDetailTableView.h"
#import "BIMTuijianDetailHeaderView.h"
#import "BIMTuijianDetailCommentCell.h"
#import "BIMTuijiandatingModel.h"
#import "BIMBuyRecordsVC.h"
...
@interface BIMTuijianDetailTableView()<UITableViewDelegate,UITableViewDataSource,TuijianDetailCommentCellDelegate,UIWebViewDelegate>
...
@end
@implementation BIMTuijianDetailTableView
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        [self registerClass:[BIMTuijianDetailHeaderView class] forCellReuseIdentifier:CellTuijianDetailHeader];
        [self registerClass:[BIMTuijianDetailHeaderView class] forCellReuseIdentifier:@"showCell"];
        [self registerClass:[BIMTuijianDetailCommentCell class] forCellReuseIdentifier:CellTuijianDetailComment];
        [self registerClass:NSClassFromString(@"BIMTuijianDatingCell") forCellReuseIdentifier:CellTuijianDating];
    }
    return self;
}
...
- (void)setPayUsersModel:(BIMpayUserModel *)payUsersModel {
...
- (void)setArrPic:(NSArray *)arrPic{
    ...
    if (_arrPic.count > 0) {
        for (int i = 0; i < _arrPic.count;  i ++) {
            BIMpayUserModel *model = _arrPic[i];
            [_picArray addObject:model.pic];
        }
    }
}
- (void)setTuijianModel:(BIMTuijiandatingModel *)tuijianModel {
    _tuijianModel = tuijianModel;
}
- (void)setHeaderModel:(BIMTuijiandatingModel *)headerModel
{
...
{  DetailGroupModel *model = _arrData[indexPath.section];
    if (indexPath.section == 0) {
        if (_typeTuijianDetailHeader == typeTuijianDetailHeaderCellDanchang) {
            if (_headerModel.see) {
                return [tableView fd_heightForCellWithIdentifier:@"showCell" configuration:^(BIMTuijianDetailHeaderView* cell) {
                    cell.webViewHight = _cellWebhight;
                    cell.model = _headerModel;
                }];
            } else {
                return [tableView fd_heightForCellWithIdentifier:CellTuijianDetailHeader configuration:^(BIMTuijianDetailHeaderView* cell) {
                    cell.webViewHight = _cellWebhight;
                    cell.model = _headerModel;
                }];
            }
        ...
        return [tableView fd_heightForCellWithIdentifier:CellTuijianDetailComment cacheByIndexPath:indexPath configuration:^(BIMTuijianDetailCommentCell* cell) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        if (_typeTuijianDetailHeader == typeTuijianDetailHeaderCellDanchang) {
            if (_headerModel.see) {
                BIMTuijianDetailHeaderView   *cell = [tableView dequeueReusableCellWithIdentifier:@"showCell" forIndexPath:indexPath];
                
                cell.selectionStyle = UITableViewCellSelectionStyleNone;
                cell.webViewHight = _cellWebhight;
                cell.model = _headerModel;
                NSLog(@"BIMTuijianDetailHeaderView666-%@1", cell.model.content);
                return cell;
            } else {
                BIMTuijianDetailHeaderView   *cell = [tableView dequeueReusableCellWithIdentifier:CellTuijianDetailHeader forIndexPath:indexPath];
                
                cell.selectionStyle = UITableViewCellSelectionStyleNone;
                cell.webViewHight = _cellWebhight;
                cell.model = _headerModel;
                NSLog(@"BIMTuijianDetailHeaderView666-%@0", cell.model.content);
                return cell;
            }
        ...
        BIMTuijianDatingCell *cell = [tableView dequeueReusableCellWithIdentifier:CellTuijianDating forIndexPath:indexPath];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        ...
        BIMTuijianDetailCommentCell *cell = [tableView dequeueReusableCellWithIdentifier:CellTuijianDetailComment forIndexPath:indexPath];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        cell.delegate = self;
        cell.type = typeCommentCellTuijian;
        if (_arrData.count >0) {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if (section == 2) {
        ...
        [lab setAttributedText:[BIMMethods withContent:lab.text WithColorText:[NSString stringWithFormat:@"%ld",_headerModel.comment_count] textColor:redcolor strFont:font14]];
...
- (void)payViewTap {
    NSLog(@"view被点击");
    if ([BIMMethods getUserModel].idId != _headerModel.user_id) {
    ...
    BIMBuyRecordsVC *buyerVC = [[BIMBuyRecordsVC alloc] init];
...
- (void)touchBasicViewToHideHudViewWithIdid:(NSInteger)Idid
{
    ...
    for (int i = 0; i<_arrCells.count; i++) {
        ...
        BIMCommentModel *model = [group.dataList objectAtIndex:i];
        if (model.Idid != Idid) {
            BIMTuijianDetailCommentCell *cell = [_arrCells objectAtIndex:i];
            [cell hideHudView];
        }
    }
}
