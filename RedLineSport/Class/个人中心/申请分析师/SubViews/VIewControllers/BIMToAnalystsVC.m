...
#import "BIMSuccessfulView.h"
#import "BIMChangePhoneNumVC.h"
#import "BIMAnalystsEventFilterVC.h"
#import "BIMPictureView.h"
@interface BIMToAnalystsVC ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate,UITextViewDelegate,SuccessfulViewDelegate,AnalystsEventFilterVCDelegate, UIActionSheetDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate>
...
@property (nonatomic, strong)BIMSuccessfulView *succView;
@property (nonatomic, strong)BIMNavView *nav;
@property (nonatomic, assign) CGFloat textViewHeight;
@property (nonatomic, strong)BIMUserModel *Newmodel;
@property (nonatomic, strong)NSString *strLeagues;
@property (nonatomic, strong)UILabel *labReason;
@property (nonatomic, strong)UIButton *sureBtn;
@property (nonatomic, strong) BIMPictureView *picView1;
@property (nonatomic, strong) BIMPictureView *picView2;
@property (nonatomic, strong) BIMPictureView *picView3;
...
@end
@implementation BIMToAnalystsVC
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    [[BIMDCHttpRequest shareInstance]sendRequestByMethod:@"post" WithParamaters:@{@"type":@"avatar"} PathUrlL:[NSString stringWithFormat:@"http://mobile.gunqiu.com:8897%@",url_uploadAliyun] ArrayFile:[NSArray arrayWithObjects:image, nil] Start:^(id requestOrignal) {
...
- (void)setModel:(BIMUserModel *)model{
    _model = model;
...
- (BIMSuccessfulView *)succView{
    if (!_succView) {
        _succView = [[BIMSuccessfulView alloc] initWithFrame:CGRectMake(0,APPDELEGATE.customTabbar.height_myNavigationBar + 10, Width, 200)];
...
- (void)sureBtnCilck{
    ...
    if (![BIMMethods isNameValid:self.txtName.text]) {
    ...
    [[BIMDCHttpRequest shareInstance] sendRequestByMethod:@"post" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_loginAndRegister] ArrayFile:nil Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            self.tableView.hidden = YES;
            ...
            [BIMMethods updateUsetModel:self.model];
            [self.tableView reloadData];
        }else{
...
- (BIMPictureView *)picView1
{
    if (!_picView1) {
        _picView1 = [[BIMPictureView alloc] init];
        _picView1.detailText = @"手持身份证";
    }
    return _picView1;
}
- (BIMPictureView *)picView2
{
    if (!_picView2) {
        _picView2 = [[BIMPictureView alloc] init];
        _picView2.detailText = @"身份证正面";
    }
    return _picView2;
}
- (BIMPictureView *)picView3
{
    if (!_picView3) {
        _picView3 = [[BIMPictureView alloc] init];
...
- (void)clickPhone:(UIButton *)btn{
    ...
    if (btn.tag == 1) {
        BIMAnalystsEventFilterVC *change = [[BIMAnalystsEventFilterVC alloc] init];
        change.hidesBottomBarWhenPushed = YES;
        change.delegate = self;
        [APPDELEGATE.customTabbar pushToViewController:change animated:YES];
    }else{
        BIMChangePhoneNumVC *change = [[BIMChangePhoneNumVC alloc] init];
        change.hidesBottomBarWhenPushed = YES;
        [APPDELEGATE.customTabbar pushToViewController:change animated:YES];
    }
}
