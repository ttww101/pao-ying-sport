#import "BIMUserViewController.h"
#import "BIMUserOfotherCellTwo.h"
#import "BIMUserTongjiCell.h"
#import "BIMUserTongjiGoodPlayCell.h"
#import "BIMTuijianDatingCell.h"
#import "BIMTuijiandatingModel.h"
#import "BIMUserTuijianVC.h"
#import "BIMUserTongjiAllModel.h"
#import "BIMWKWebView.h"
#import "BIMTongjiVC.h"
#import "BIMTopContentView.h"
...
@interface BIMUserViewController ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate,UserOfotherCellTwoDelegate,UserTongjiCellDelegate, GQWebViewDelegate, TopContentViewDelegate, WKNavigationDelegate>
@property (nonatomic, strong) BIMUserModel *userModel;
@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, strong) BIMUserOfotherCellTwo *usercell;
@property (nonatomic, strong) NSArray *arrRecommand;
@property (nonatomic, strong) BIMUserTongjiAllModel *tongjiModel;
@property (nonatomic, assign) BOOL showMoreUserInfo;
@property (nonatomic, strong) BIMNavView *nav;
@property (nonatomic , strong) BIMTopContentView *topView;
@property (nonatomic, assign) BOOL isBack;
@property (nonatomic , strong) BIMWKWebView *webView;
@end
@implementation BIMUserViewController
...
//    model.webUrl = [NSString stringWithFormat:@"https://apple.com", APPDELEGATE.url_ip,H5_Host,_userId];
    BIMWKWebView *wkWebView = [[BIMWKWebView alloc]init];
...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        [_tableView registerClass:[BIMUserOfotherCellTwo class] forCellReuseIdentifier:cellUserViewController];
        [_tableView registerClass:[UITableViewCell class] forCellReuseIdentifier:@"cell"];
        [_tableView registerClass:[BIMTuijianDatingCell class] forCellReuseIdentifier:cellUserViewControllerRecommand];
        [_tableView registerClass:[BIMUserTongjiCell class] forCellReuseIdentifier:cellUserTongji];
        [_tableView registerClass:[BIMUserTongjiGoodPlayCell class] forCellReuseIdentifier:cellUserTongjiGoodPlay];
...
- (void)didToTongjiVC
{
    BIMTongjiVC *tongji = [[BIMTongjiVC alloc] init];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        switch (indexPath.row) {
            case 0:
            {
                return [tableView fd_heightForCellWithIdentifier:cellUserViewController configuration:^(BIMUserOfotherCellTwo * cell) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        switch (indexPath.row) {
            case 0:
            {
                BIMUserOfotherCellTwo *cell = [tableView dequeueReusableCellWithIdentifier:cellUserViewController];
                if (!cell) {
                    cell = [[BIMUserOfotherCellTwo alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellUserViewController];
                }
            ...
            case 1:
            {
                BIMUserTongjiCell *cell = [tableView dequeueReusableCellWithIdentifier:cellUserTongji];
                if (!cell) {
                    cell = [[BIMUserTongjiCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellUserTongji];
                }
                cell.selectionStyle = UITableViewCellSelectionStyleNone;
            ...
            case 2:
            {
                BIMUserTongjiGoodPlayCell *cell = [tableView dequeueReusableCellWithIdentifier:cellUserTongjiGoodPlay];
                if (!cell) {
                    cell = [[BIMUserTongjiGoodPlayCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellUserTongjiGoodPlay];
                }
                cell.selectionStyle = UITableViewCellSelectionStyleNone;
                if (_tongjiModel) {
        ...
        BIMTuijianDatingCell *cell = [tableView dequeueReusableCellWithIdentifier:cellUserViewControllerRecommand];
        if (!cell) {
            cell = [[BIMTuijianDatingCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellUserViewControllerRecommand];
        }
...
{
    NSString*UBIMtring=[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_userinfo];
    ...
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:UBIMtring Start:^(id requestOrignal) {
        if (!_userModel) {
...
- (void)addAtention:(BOOL)attention {
    ...
    [[BIMDCHttpRequest shareInstance] sendRequestByMethod:@"post" WithParamaters:paremeter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url] ArrayFile:nil Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
...
- (void)lodaRecommandData
{
    ...
    NSString*UBIMtring;
    if (self.Number==1) {
        UBIMtring=[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_recommendlistUser];
        [parameter setObject:[NSString stringWithFormat:@"%d",0] forKey:@"oddstype"];
    }else{
        UBIMtring=[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_quizmyQuizList];
    ...
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:UBIMtring Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            _arrRecommand = [NSArray arrayWithArray:[BIMTuijiandatingModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"data"]]] ;
            [self.tableView reloadData];
        }else{
        }
    } Failure:^(NSError *error, NSString *errorDict, id responseOrignal) {
...
- (void)loadTongjiData
{
    NSString*UBIMtring;
    if (self.Number==1) {
        UBIMtring=[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_ucenterstatis];
    }else{
        UBIMtring=[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_quizmyQuizIndex];
    }
    NSMutableDictionary *parameter = [NSMutableDictionary dictionaryWithDictionary:[BIMHttpString getCommenParemeter]];
    ...
    [[BIMDCHttpRequest shareInstance]sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:UBIMtring Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            _tongjiModel = [BIMUserTongjiAllModel entityFromDictionary:[responseOrignal objectForKey:@"data"]];
            [self.tableView reloadData];
        }else{
        }
    } Failure:^(NSError *error, NSString *errorDict, id responseOrignal) {
...
- (BIMTopContentView *)topView {
    if (_topView == nil) {
        _topView = [[BIMTopContentView alloc]init];
