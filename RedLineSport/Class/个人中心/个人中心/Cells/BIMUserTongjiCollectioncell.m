...
#import "BIMSetValueFormatter.h"
#import "BIMUserTuijianNumView.h"
#import "BIMRoundUserView.h"
#import "BIMUserTongjiCollectioncell.h"
@interface BIMUserTongjiCollectioncell ()<ChartViewDelegate>
@property (nonatomic,strong) LineChartView * lineView;
@property (nonatomic,strong) UILabel * markY;
@end
@implementation BIMUserTongjiCollectioncell
- (void)setModel:(BIMUserTongjiModel *)model
{
    _model = model;
    ...
    BIMRoundUserView *roundWin = [[BIMRoundUserView alloc] initWithFrame:CGRectMake(15, 10, 100*scaleRoundWidth, 100*scaleRoundWidth)];
    ...
    BIMRoundUserView *roundProfit = [[BIMRoundUserView alloc] initWithFrame:CGRectMake(roundWin.right + 10, 10, 100*scaleRoundWidth, 100*scaleRoundWidth)];
    ...
    BIMUserTuijianNumView *numV = [[BIMUserTuijianNumView alloc] initWithFrame:CGRectMake(Width - 15 - 100*scaleRoundWidth, 0, 100*scaleRoundWidth, 96)];
    numV.model = _model;
...
- (LineChartView *)lineView {
    if (!_lineView) {
        ...
        leftAxis.valueFormatter = [[BIMSymbolsValueFormatter alloc]init];
...
- (LineChartData *)setData{
    ...
    for (int i = 0; i<_model.groupTimeStatis.count; i++) {
        BIMTotalrateModel *model = [_model.groupTimeStatis objectAtIndex:i];
    ...
    _lineView.xAxis.valueFormatter = [[BIMDateValueFormatter alloc]initWithArr:arrXlabs];
    LineChartDataSet *set1 = nil;
    if (_lineView.data.dataSetCount > 0) {
        LineChartData *data = (LineChartData *)_lineView.data;
        set1 = (LineChartDataSet *)data.dataSets[0];
        set1.values = arrValueProfit;
        set1.valueFormatter = [[BIMSetValueFormatter alloc]initWithArr:arrValueProfit];
        return data;
    }else{
