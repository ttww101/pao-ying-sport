...
#import "BIMMessageControl.h"
#import "BIMDC_JZAPhotoVC.h"
#import "BIMUserTuijianVC.h"
@interface BIMTopContentView ()
...
@implementation BIMTopContentView
...
- (void)setModel:(BIMUserModel *)model {
    ...
    if (self.recordArray.count > 0) {
        [self.recordArray enumerateObjectsUsingBlock:^(BIMMessageControl *obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    for (NSInteger i = 0; i < titleArray.count; i ++) {
        BIMMessageControl *control = [[BIMMessageControl alloc]initWithFrame:CGRectMake(60 + i * width, self.height - 60, width, 60) title:titleArray[i] amount:[contentArray[i] stringValue]];
...
- (void)controlAction:(BIMMessageControl *)sender {
    if (sender.tag == 0) {
