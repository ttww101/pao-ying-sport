#import "BIMUserTongjiView.h"
@interface BIMUserTongjiView()
...
@end
@implementation BIMUserTongjiView
...
- (void)setModel:(BIMStatisticsModel *)model
{
    _model = model;
    ...
    if ([_model.win_rate floatValue]>0) {
        if ([_labWinRate.text containsString:@"."]) {
            _labWinRate.attributedText = [BIMMethods withContent:_labWinRate.text WithColorText:[_labWinRate.text substringFromIndex:_labWinRate.text.length - 3] textColor:redcolor strFont:font12];
        } else {
             _labWinRate.attributedText = [BIMMethods withContent:_labWinRate.text WithColorText:@"%" textColor:redcolor strFont:font12];
        }
        _labWinRate.textColor = redcolor;
    }else{
        if ([_labWinRate.text containsString:@"."]) {
            _labWinRate.attributedText = [BIMMethods withContent:_labWinRate.text WithColorText:[_labWinRate.text substringFromIndex:_labWinRate.text.length - 3] textColor:color99 strFont:font12];
        } else {
            _labWinRate.attributedText = [BIMMethods withContent:_labWinRate.text WithColorText:@"%" textColor:color99 strFont:font12];
        }
        _labWinRate.textColor = color99;
    }
    if ([_model.profit_rate floatValue]>0) {
        _labProfite.textColor = redcolor;
        _labProfite.attributedText = [BIMMethods withContent:_labProfite.text WithColorText:[_labProfite.text substringFromIndex:_labProfite.text.length - 3] textColor:redcolor strFont:font12];
    }else{
        _labProfite.textColor = color99;
        _labProfite.attributedText = [BIMMethods withContent:_labProfite.text WithColorText:[_labProfite.text substringFromIndex:_labProfite.text.length - 3] textColor:color99 strFont:font12];
    }
    _roundTitle.text = @"共";
    _winTitle.text = @"场，赢";
    _zouTitle.text = @"走";
