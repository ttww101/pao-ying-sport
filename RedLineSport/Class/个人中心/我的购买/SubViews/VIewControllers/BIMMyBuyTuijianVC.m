...
@interface BIMMyBuyTuijianVC ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
@property (nonatomic, strong) UITableView *tableView;
@property (nonatomic, strong) NSMutableArray *arrdata;
@property (nonatomic, assign) NSInteger limitStart;
@end
@implementation BIMMyBuyTuijianVC
...
- (UITableView *)tableView
{
    if (!_tableView) {
        ...
        [_tableView registerClass:[BIMTuijianDatingCell class] forCellReuseIdentifier:cellMyBuyTuijianVC];
...
{
    return [tableView fd_heightForCellWithIdentifier:cellMyBuyTuijianVC cacheByIndexPath:indexPath configuration:^(BIMTuijianDatingCell *cell) {
        if (_arrdata.count>0) {
...
{
    BIMTuijianDatingCell *cell = [tableView dequeueReusableCellWithIdentifier:cellMyBuyTuijianVC];
    if (!cell) {
        cell = [[BIMTuijianDatingCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellMyBuyTuijianVC];
    }
...
- (void)lodaRecommandDataType:(loadDataType)loadType
{
    ...
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_buy_listUser] Start:^(id requestOrignal) {
