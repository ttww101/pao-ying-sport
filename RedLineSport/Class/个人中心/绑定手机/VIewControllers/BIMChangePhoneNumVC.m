...
@property (nonatomic, strong)BIMJKCountDownButton *btn;
...
@end
@implementation BIMChangePhoneNumVC
...
- (void)sureBtnCilck{
    ...
    if (self.txtOldNum.text.length != 11 || ![BIMMethods isMobileNumber:self.txtOldNum.text]) {
        [SVProgressHUD showImage:[UIImage imageNamed:@""] status:@"旧手机号有误"];
        return;
    }
    if (self.txtNewNum.text.length != 11 || ![BIMMethods isMobileNumber:self.txtNewNum.text]) {
...
- (BIMJKCountDownButton *)btn{
    if (!_btn) {
        _btn = [BIMJKCountDownButton buttonWithType:UIButtonTypeCustom];
