...
@interface BIMUserTuijianTable()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
@property (nonatomic, strong) NSMutableArray *arrData;
@property (nonatomic ,assign) NSInteger currentLimitStart;
@end
@implementation BIMUserTuijianTable
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        [self registerClass:[BIMTuijianDatingCell class] forCellReuseIdentifier:cellUserTuijianTable];
        self.delegate =self;
...
{
    BIMTuijianDatingCell *cell = [tableView dequeueReusableCellWithIdentifier:cellUserTuijianTable];
    if (!cell) {
        cell = [[BIMTuijianDatingCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellUserTuijianTable];
    }
...
- (void)lodaDataWithLoadtype:(loadDataType)loadDateType
{
    ...
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_newrecommendlistUser] Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
        [self.mj_header endRefreshing];
    } Success:^(id responseResult, id responseOrignal) {
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            self.defaultTitle = @"暂无推荐，你要做头条吗";
            NSArray *arr =[NSArray arrayWithArray:[BIMTuijiandatingModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"data"]]];
            if (arr.count != 0) {
