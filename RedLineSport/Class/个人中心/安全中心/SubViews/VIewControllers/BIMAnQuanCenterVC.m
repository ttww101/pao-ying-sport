...
@interface BIMAnQuanCenterVC ()<UITableViewDelegate,UITableViewDataSource>
@property(nonatomic, strong)UITableView *tableView;
@end
@implementation BIMAnQuanCenterVC
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        switch (indexPath.row) {
            case 0:{
                BIMRealNameCerVC *realNameVC = [[BIMRealNameCerVC alloc] init];
                realNameVC.type = 0;
                ...
                break;
            case 1:{
                BIMBangAccountVC *bangVC = [[BIMBangAccountVC alloc] init];
                bangVC.hidesBottomBarWhenPushed = YES;
                [APPDELEGATE.customTabbar pushToViewController:bangVC animated:YES];
            }
                break;
            case 2:{
                BIMChangePassWordVC *psssWordVC = [[BIMChangePassWordVC alloc] init];
