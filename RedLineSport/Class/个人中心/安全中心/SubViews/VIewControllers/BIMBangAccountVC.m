...
@interface BIMBangAccountVC ()<UITableViewDelegate,UITableViewDataSource>
...
@end
@implementation BIMBangAccountVC
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0) {
        switch (indexPath.row) {
            case 0:
            {
                BIMChangePhoneNumVC *phoneVC = [[BIMChangePhoneNumVC alloc] init];
...
- (void)isAccountBang {
    ...
    [[BIMDCHttpRequest shareInstance] sendRequestByMethod:@"post" WithParamaters:dictParameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_hasAccountBanged] ArrayFile:nil Start:^(id requestOrignal) {
...
- (void)bangAccountWithUserName:(NSString *)username nickName:(NSString *)nickname resource:(NSString *)resource
{
    ...
    [[BIMDCHttpRequest shareInstance] sendRequestByMethod:@"post" WithParamaters:dictParameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_bang_Account] ArrayFile:nil Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            [self isAccountBang];
