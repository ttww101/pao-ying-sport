...
#import "BIMItemControl.h"
@interface BIMHeaderAmountView ()
...
@end
@implementation BIMHeaderAmountView
...
- (void)setModel:(BIMUserModel *)model {
    ...
    for (NSInteger i = 0; i < itemArray.count; i ++) {
        ...
        BIMItemControl *control  = [[BIMItemControl alloc]initWithFrame:CGRectMake(i * itemWidth, 0, itemWidth, self.height) imageName:dic[@"icon"] title:dic[@"title"] amount:str hidenLine:i];
        control.tag = i;
...
#pragma mark - Events
- (void)controlAction:(BIMItemControl *)senter {
    switch (senter.tag) {
        case 0: {
            [self myDiamond];
        }
