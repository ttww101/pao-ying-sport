...
#import "BIMRegisterViewController.h"
#import "MBProgressHUD.h"
#import "BIMDCHttpRequest.h"
#import "BIMJKCountDownButton.h"
#import <UMPush/UMessage.h>

#define colorBtn ColorWithRGBA(253, 186, 49, 1)
@interface BIMRegisterViewController ()<UITextFieldDelegate>
{
...
@property (nonatomic, strong) BIMJKCountDownButton *checkBtn;
...
@implementation BIMRegisterViewController
...
//    _checkTextF.borderStyle = UITextBorderStyleRoundedRect;
    ...
    _checkBtn = [[BIMJKCountDownButton alloc] initWithFrame:CGRectMake(_checkTextF.frame.origin.x + _checkTextF.frame.size.width - 10, textHeight + 10, 80, textHeight - 20)];
...
- (void)argementlabClick {
   ...
   // 用户服务协议
    ...
    BIMWKWebViewController *webVC = [[BIMWKWebViewController alloc] init];
...
- (void)getCheckCode:(BIMJKCountDownButton *)btn
{
    ...
    [btn didChange:^NSString *(BIMJKCountDownButton *countDownButton, int second) {
        return [NSString stringWithFormat:@"剩余%d秒",second];
    }];
    [btn didFinished:^NSString *(BIMJKCountDownButton *countDownButton, int second) {
...
- (void)getcheckCode
{
     ...
     */
    [self.view endEditing:YES];
    NSMutableDictionary *dictParameter = [NSMutableDictionary dictionaryWithDictionary:[BIMHttpString getCommenParemeter]];
    ...
    [[BIMDCHttpRequest shareInstance] sendRequestByMethod:@"post" WithParamaters:dictParameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_loginAndRegister] ArrayFile:nil Start:^(id requestOrignal) {
...
- (void)registerUser
{
    ...
    NSMutableDictionary *dictPatameter = [NSMutableDictionary dictionaryWithDictionary:[BIMHttpString getCommenParemeter]];
    ...
    [dictPatameter setObject:[BIMMethods md5WithString:_pswTextF.text] forKey:@"password"];
    ...
    [[BIMDCHttpRequest shareInstance] sendRequestByMethod:@"post" WithParamaters:dictPatameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_loginAndRegister] ArrayFile:nil Start:^(id requestOrignal) {
        if (!_prograssHud) {
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            ...
            BIMUserModel *model = [BIMUserModel entityFromDictionary:[responseOrignal objectForKey:@"data"]];
            BIMTokenModel *tokenModel = [BIMTokenModel entityFromDictionary:[responseOrignal objectForKey:@"data"]];
            ...
            [BIMMethods updateUsetModel:model];
            [[NSUserDefaults standardUserDefaults] synchronize];
...
- (void)connectRongyunWithUserModel:(BIMUserModel *)user
{}
//{
...
//    [[BIMDCHttpRequest shareInstance] sendRequestByMethod:@"post" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_getRonyunUserToken] ArrayFile:nil Start:^(id requestOrignal) {
//        
//    } End:^(id responseOrignal) {
//        
//    } Success:^(id responseResult, id responseOrignal) {
