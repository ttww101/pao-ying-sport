...
#import "BIMBangUserVC.h"
#import <UMPush/UMessage.h>
@interface BIMLoginViewController ()<UITextFieldDelegate>
{
...
- (void)navViewTouchAnIndex:(NSInteger)index
{
    if (index == 1) {
        ...
        BIMRegisterViewController *registerVC = [[BIMRegisterViewController alloc]init];
...
{
    BIMForgetPswViewController *forgetPswVC = [[BIMForgetPswViewController alloc] init];
...
- (void)login
{
    ...
    [dictParameter setObject:[BIMMethods md5WithString:_pswTextF.text] forKey:@"password"];
...
- (void)loginThirdPartWithUserName:(NSString *)username nickName:(NSString *)nickname picUrl:(NSString *)pic resource:(NSString *)resource password:(NSString *)password
{
    ...
    [dictParameter setObject:[BIMMethods md5WithString:password] forKey:@"password"];
    [dictParameter setObject:@"1" forKey:@"platform"];
    ...
    [[BIMDCHttpRequest shareInstance] sendRequestByMethod:@"post" WithParamaters:dictParameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_loginAndRegister] ArrayFile:nil Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            ...
            NSString *nowDate = [BIMMethods getDateByStyle:dateStyleFormatter withDate:[NSDate dateWithTimeIntervalSinceNow:3600*24*15]];
...
{
    [self.view endEditing:YES];
    BIMBangUserVC *bangVC = [BIMBangUserVC new];
