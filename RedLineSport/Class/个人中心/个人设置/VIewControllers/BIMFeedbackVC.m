...
#import "BIMFeedBackHeaderView.h"
#import "BIMFeedBackCell.h"
#import "BIMFeedBackModel.h"
#import "BIMTFFileUploadManager.h"
#import "AFNetworking.h"
@interface BIMFeedbackVC ()<UITextViewDelegate,UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
...
@property (nonatomic, strong) BIMFeedBackHeaderView                *feedBackHeaderView;
@property (nonatomic, strong) NSMutableArray                   *feedArr;
@property (nonatomic, strong) NSString                          *uBIMtr;
@end
@implementation BIMFeedbackVC
...
- (void)setupSubView
{
    ...
    if ([BIMMethods login]) {
        _telPhoneTextField.text = [BIMMethods getUserModel].showmobile;
    }else{
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:[BIMFeedBackCell class] forCellReuseIdentifier:@"feedBackCell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    BIMFeedBackCell *feedBackCell = [tableView cellForRowAtIndexPath:indexPath];
    if (!feedBackCell) {
        feedBackCell = [[BIMFeedBackCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"feedBackCell"];
    }
...
- (void)getComments {
    ...
    [[BIMDCHttpRequest shareInstance] sendRequestByMethod:@"post" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_feedback] ArrayFile:nil Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            _feedArr = [[NSMutableArray alloc] initWithArray:[BIMFeedBackModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"data"]]];
            [self.tableView reloadData];
        }else{
...
- (void)upLoadImage {
    ...
    [[BIMDCHttpRequest shareInstance] sendRequestByMethod:@"post" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_upLoadImg,url_feedBack_upLoadImg] ArrayFile:[NSArray arrayWithObjects:_image, nil]  FileName:@"imagefile"
      Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            _uBIMtr = [[responseOrignal objectForKey:@"data"] objectForKey:@"picurl"];
            [self feedBack];
        }else{
            [SVProgressHUD showImage:[UIImage imageNamed:@""] status:[responseOrignal objectForKey:@"msg"]];
        }
...
- (void)feedBack
{
    ...
    if (!isNUll(_uBIMtr)) {
        [parameter setObject:_uBIMtr forKey:@"imgurl"];
    }
