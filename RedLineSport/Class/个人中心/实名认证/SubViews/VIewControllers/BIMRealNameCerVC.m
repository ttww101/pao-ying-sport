#import "BIMRealNameCerVC.h"
#import "BIMSuccessfulView.h"
@interface BIMRealNameCerVC ()
...
@end
@implementation BIMRealNameCerVC
...
- (void)viewDidLoad {
    ...
    _model = [BIMMethods getUserModel];
    _timeNUm = 3;
...
- (void)sureBtnCilck{
    ...
    if (_textName.text.length == 0 || ![BIMMethods isNameValid:_textName.text]) {
        [SVProgressHUD showImage:[UIImage imageNamed:@""] status:@"真实姓名有误"];
        return;
    }
    if (![BIMMethods chk18PaperId:_textCarNum.text]) {
    ...
    [[BIMDCHttpRequest shareInstance] sendRequestByMethod:@"post" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_loginAndRegister] ArrayFile:nil Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            ...
            [BIMMethods updateUsetModel:_model];
...
- (BIMSuccessfulView *)succView{
    if (!_succView) {
        ...
        _succView.labContent.attributedText = [BIMMethods withContent:_succView.labContent.text contentColor:color99 WithColorText:@"3秒" textColor:redcolor];
...
- (void)delayMethod{
    ...
    self.succView.labContent.attributedText = [BIMMethods withContent:self.succView.labContent.text contentColor:color99 WithColorText:[NSString stringWithFormat:@"%ld秒",_timeNUm] textColor:redcolor];
    if (_timeNUm == 0) {
        [_timerl invalidate];
        [self.navigationController popViewControllerAnimated:YES];
    }
...
- (UILabel *)labStr{
    if (!_labStr) {
        ...
        _labStr.text = @"真实姓名和身份证是结算和认证的依据，请填写真实信息，认证后不可修改";
         [_labStr setAttributedText:[BIMMethods setTextStyleWithString:_labStr.text WithLineSpace:4 WithHeaderIndent:0]];
    }
    return _labStr;
}
- (void)setAotView{
