#import "BIMRemindViewController.h"
#import "BIMRemindModel.h"
#import "BIMTuijianDetailVC.h"
#define cellRemindViewController @"cellRemindViewController"
@interface BIMRemindViewController ()<UITableViewDelegate,UITableViewDataSource,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
@property (nonatomic, strong) BIMBasicTableView *tableView;
@property (nonatomic, strong) NSArray *arrData;
@end
@implementation BIMRemindViewController
...
{
    if (!_tableView) {
        self.tableView = [[BIMBasicTableView alloc] initWithFrame:CGRectMake(0, APPDELEGATE.customTabbar.height_myNavigationBar, Width, Height - APPDELEGATE.customTabbar.height_myNavigationBar) style:UITableViewStylePlain];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BIMRemindModel *model = [_arrData objectAtIndex:indexPath.row];
    ...
    labtime.font = font10;
    labtime.text = [BIMMethods timeToNowWith:model.time];
    ...
    if (!model.iread) {
        ...
        labTitle.attributedText = [BIMMethods withContent:labTitle.text WithColorText:@"发布新推荐" textColor:color66 strFont:font12];
    }else{
...
-  (void)loadUreadRemind
{
    ...
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_noticedo] Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            _arrData = [[NSArray alloc] initWithArray:[BIMRemindModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"data"]]];
...
-  (void)updateNoticeWithNoticeId:(BIMRemindModel *)model
{
    NSMutableDictionary *parameter = [NSMutableDictionary dictionaryWithDictionary:[BIMHttpString getCommenParemeter]];
