#import "BIMNoticeViewController.h"
#import "BIMNoticeModel.h"
#import "BIMNoticeDetailViewController.h"
#define cellNotice @"cellNotice"
@interface BIMNoticeViewController ()<UITableViewDataSource,UITableViewDelegate,DZNEmptyDataSetSource,DZNEmptyDataSetDelegate>
@property (nonatomic, strong) BIMBasicTableView *tableView;
@property (nonatomic, strong) NSArray *arrData;
@end
@implementation BIMNoticeViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BIMNoticeModel *model = [_arrData objectAtIndex:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    BIMNoticeDetailViewController *noticeDVC = [[BIMNoticeDetailViewController alloc] init];
...
-  (void)loadUreadNotification
{
    ...
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_noticedo] Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            _arrData = [[NSArray alloc] initWithArray:[BIMNoticeModel arrayOfEntitiesFromArray:[responseOrignal objectForKey:@"data"]]];
...
-  (void)updateNoticeWithNoticeId:(BIMNoticeModel *)model
{
    NSMutableDictionary *parameter = [NSMutableDictionary dictionaryWithDictionary:[BIMHttpString getCommenParemeter]];
