...
#import "BIMTabbarButton.h"
@implementation BIMCustmerTableBar
- (instancetype)init {
    self = [super init];
    if (self) {
        self.selected = 0;
...
- (void)setItemsArr:(NSArray *)itemsArr {
    ...
    BIMTabbarButton *defultSelectBtn = nil;
    for (int i = 0; i < _itemsArr.count; i++) {
        NSDictionary *dic = itemsArr[i];
        CGFloat itemsWith = self.frame.size.width / _itemsArr.count;
        BIMTabbarButton *tabbar = [[BIMTabbarButton alloc] initWithFrame:CGRectMake(itemsWith * i, 0, itemsWith, self.frame.size.height)];
...
- (void)clickTabButton:(BIMTabbarButton *)clickBtn {
    ...
    if ([self.delegate respondsToSelector:@selector(tabBar:selectedFrom:to:)]) {
        ...
        if (success) {
            for (BIMTabbarButton *btn in self.subviews) {
                btn.selected = NO;
            }
