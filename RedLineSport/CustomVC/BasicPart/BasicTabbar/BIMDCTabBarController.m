...
@interface BIMDCTabBarController ()<UIGestureRecognizerDelegate, TabBarDelegate>
{
    BIMDCNavViewController *_firstNav;
    BIMDCNavViewController *_secondNav;
    BIMDCNavViewController *_thirdNav;
    BIMDCNavViewController *_forthNav;
    BIMDCNavViewController *_mineNav;
}
...
@property (nonatomic , strong) BIMCustmerTableBar *taBar;
@property (nonatomic, assign) BOOL isToFenxi;
@end
static CGFloat imageHeight = 66.f;
@implementation BIMDCTabBarController
...
- (void)update {
    ...
    [[BIMDCHttpRequest shareInstance]sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_update] Start:^(id requestOrignal) {
...
- (void)configActivityEntrance {
    ...
    for (NSDictionary *dic in activityArray) {
        if (dic[@"main"]) {
            ...
            BIMDCNavViewController *nav = [self.childViewControllers objectAtIndex:2];
...
-  (void)loadUreadNotificationNum
{
    ...
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_noticedo] Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
...
{
    NSMutableDictionary *parameter = [NSMutableDictionary dictionaryWithDictionary:[BIMHttpString getCommenParemeter]];
    NSString *refreshToken = [BIMMethods getTokenModel].refreshToken;
    if (!refreshToken) {
        refreshToken = @"";
    }
    [parameter setObject:refreshToken forKey:@"refreshToken"]; 
    [[BIMDCHttpRequest shareInstance] sendRequestByMethod:@"post" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_refreshtoken] ArrayFile:nil Start:^(id requestOrignal) {
    } End:^(id responseOrignal) {
    } Success:^(id responseResult, id responseOrignal) {
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            ...
            BIMTokenModel *tokenModel =  [BIMMethods getTokenModel];
            tokenModel.token = tokenStr;
            tokenModel.refreshToken = refreshTokenStr;
            [BIMMethods updateTokentModel:tokenModel];
...
- (void)loadTableBarViewWithArray:(NSMutableArray *)array {
    ...
    for (UIView *view in [self.tabBar subviews]) {
        if ([view isKindOfClass:[BIMCustmerTableBar class]]) {
...
- (void)addChildControllerWithVcStr:(NSString *)vcStr
    ...
    BIMDCNavViewController *nav = [[BIMDCNavViewController alloc] initWithRootViewController:target];
...
- (void)addChildWebControllerWithVcStr:(NSString *)vcStr
                         ...
                         selectedImage:(UIImage *)selectedImage
                                 title:(NSString *)title
                                   tag:(NSInteger)tag
                              webModel:(BIMWebModel *)model {
    UIEdgeInsets insets = UIEdgeInsetsMake(Zero, Zero, Zero, Zero);
    Class targetClass = NSClassFromString(vcStr);
    BIMToolWebViewController *target = [[targetClass alloc]init];
    target.tabBarItem.title = title;
    target.model = model;
    target.tabBarItem.image = defaultImage;
    target.tabBarItem.selectedImage = selectedImage;
...
- (void)p_didSelectCenterTabBarItem
{
    ...
    Class targetCalss = NSClassFromString([NSString stringWithFormat:@"BIM%@",self.activityDic[@"n"]]);
...
- (void)pushToNewsWeb:(NSNotification *)nofication
{
    NSDictionary *pushInfo = nofication.userInfo;
     UIViewController *currentControl = [BIMMethods help_getCurrentVC];
    if ([pushInfo objectForKey:@"catalog"]== nil) {
        ...
        switch (eventID) {
            ...
            case 3://新闻呢
            {
                BIMWebDetailViewController *webDetailVC = [[BIMWebDetailViewController alloc] init];
