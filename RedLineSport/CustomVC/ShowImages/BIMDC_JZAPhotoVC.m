...
#import "BIMDCPhotoView.h"
#define screen_width [UIScreen mainScreen].bounds.size.width
#define screen_height [UIScreen mainScreen].bounds.size.height
@interface BIMDC_JZAPhotoVC ()<UIScrollViewDelegate,PhotoViewDelegate>
{
    CGFloat lastScale;
    NSMutableArray *_subViewList;
}
@end
@implementation BIMDC_JZAPhotoVC
- (id)init
{
    self = [super init];
    if (self) {
...
-(void)loadPhote:(NSInteger)index{
    ...
    if (![currentPhotoView isKindOfClass:[BIMDCPhotoView class]]) {
        CGRect frame = CGRectMake(index*_scrollView.frame.size.width, 0, self.view.frame.size.width, self.view.frame.size.height);
        BIMDCPhotoView *photoV = [[BIMDCPhotoView alloc] initWithFrame:frame withPhotoUrl:[self.imgArr objectAtIndex:index]];
