...
+ (BIMUserModel *)getUserModel
{
    NSArray *documents = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documentsPath = documents[0];
    BIMUserModel *model = [NSKeyedUnarchiver unarchiveObjectWithFile:[documentsPath stringByAppendingPathComponent:@"user.plist"]];
    return model;
}
+ (void)updateUsetModel:(BIMUserModel *)model
{
    NSArray *documents = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documentsPath = documents[0];
    [NSKeyedArchiver archiveRootObject:model toFile:[documentsPath  stringByAppendingPathComponent:@"user.plist"]];
}
+ (BIMTokenModel *)getTokenModel
{
    NSArray *documents = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documentsPath = documents[0];
    BIMTokenModel *model = [NSKeyedUnarchiver unarchiveObjectWithFile:[documentsPath stringByAppendingPathComponent:@"token.plist"]];
    return model;
}
+ (void)updateTokentModel:(BIMTokenModel *)model
{
    NSArray *documents = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documentsPath = documents[0];
...
+ (BOOL)login
{
    if ([[NSUserDefaults standardUserDefaults] boolForKey:islogin]) {
        if ([[NSUserDefaults standardUserDefaults] boolForKey:@"thirdPartLogin"]) {
            ...
            NSDate *thirdPartLoginDeadline = [BIMMethods getDateFromString:[[NSUserDefaults standardUserDefaults] objectForKey:@"thirdPartLoginDeadline"] byformat:dateStyleFormatter];
...
+ (void)toLogin
{
    BIMLoginViewController *loginVC = [[BIMLoginViewController alloc] init];
