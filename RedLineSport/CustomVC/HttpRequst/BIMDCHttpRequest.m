...
@interface BIMDCHttpRequest ()
@property (nonatomic, strong) UIView *animateBasicView;
@end
@implementation BIMDCHttpRequest
{
}
+ (BIMDCHttpRequest *)shareInstance
{
    static BIMDCHttpRequest *dcHttpRequset = nil;
    static dispatch_once_t onceInitDCHttpRequest;
    dispatch_once(&onceInitDCHttpRequest, ^{
        dcHttpRequset = [[BIMDCHttpRequest alloc] init];
    });
    if ([BIMMethods login]) {
        [_afnetManager.requestSerializer setValue:[BIMMethods getTokenModel].token forHTTPHeaderField:@"token"];
    }else{
...
+ (BIMDCHttpRequest *)guestInstance {
    static BIMDCHttpRequest *dcHttpRequset = nil;
    static dispatch_once_t onceInitDCHttpRequest;
    dispatch_once(&onceInitDCHttpRequest, ^{
        dcHttpRequset = [[BIMDCHttpRequest alloc] init];
