...
@interface BIMAppleIAPService () <SKProductsRequestDelegate, SKPaymentTransactionObserver>
...
@end
@implementation BIMAppleIAPService
+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    static BIMAppleIAPService *service = nil;
    dispatch_once(&onceToken, ^{
        service = [[BIMAppleIAPService alloc]init];
    });
    return service;
}
- (instancetype)init
...
-(void)uploadReceipt:(SKPaymentTransaction *)transaction {
    ...
    [[BIMDCHttpRequest shareInstance] sendRequestByMethod:@"post" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_verifyPayment]  ArrayFile:nil Start:^(id requestOrignal) {
...
-(void)uploadSanBoxReceipt:(NSData *)requestData receipt:(NSString *)receipt {
    NSString *verifyUBIMtring = @"https://sandbox.itunes.apple.com/verifyReceipt";
    NSMutableURLRequest *storeRequest = [NSMutableURLRequest requestWithURL:[[NSURL alloc] initWithString:verifyUBIMtring] cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:10.0f];
