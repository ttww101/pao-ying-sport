...
#import "BIMAppConfig.h"
#import "BIMTabConfig.h"
#import "ArchiveFile.h"
#import "BIMStartViewController.h"
...
- (void)resumePuchase {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        [[BIMAppleIAPService sharedInstance]VerifyingLocalCredentialsWithBlock:^(NSString *message, NSError *error) {
            if (error) {
...
- (void)configPageStatistics {
    NSMutableDictionary *dic = [BIMAspectManager GQ_PathForPageDic];
    if (dic) {
        NSString *lastVersion = dic[@"appVersion"];
        NSString *currentVersion = [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleShortVersionString"];
        if (![lastVersion isEqualToString:currentVersion]) {
            [BIMAspectManager GQ_SavePageDic];
        }
    } else {
        [BIMAspectManager GQ_SavePageDic];
    }
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    NSLog(@"%@",[[[NSFileManager defaultManager] UBIMForDirectory:NSDocumentDirectory inDomains:NSUserDomainMask] lastObject]);

    // 根视图 默认是透明的,也就是黑色的
    self.window.backgroundColor = [UIColor whiteColor];
    
    BIMStartViewController *launchControl = [[BIMStartViewController alloc]init];
...
//    [self initLocalNotification];
    ...
    [[BIMAppConfig shareInstance]initialize];
...
- (void)loadConfig {
    ...
    [[BIMDCHttpRequest shareInstance]sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_ConfigjSon] Start:^(id requestOrignal) {
...
- (void)setUpMessageSound {
    ...
    //设置系统音效
    [BIMMessageSoundMgr sharedInstanceForSound];
    //设置系统震动
    [BIMMessageSoundMgr sharedInstanceForVibrate];
}
...
//                    [[BIMDependetNetMethods sharedInstance] uploadImageWithImageArr:imageArr completion:^(BOOL finished, NSArray *arrUrl) {
//                        
//                        if (finished) {
//                            
...
- (void)initRootViewCotroller
{
    ...
    BIMTabConfig *config = [[BIMTabConfig alloc]init];
    ...
    [self loadFirstData];
    //第二天打开app的时候重新初始化根视图
//    NSString *currentLocalDay = [BIMMethods getDateByStyle:DateFormatter withDate:[NSDate date]];
...
- (void)applicationWillEnterForeground:(UIApplication *)application {
    ...
    if ([BIMMethods login]) {      // 用户登出，清除 OutOfRefreshTokenTime / refreshTokentime
...
//    [BIMCacheObject delete_Data_urlKey:@"jiaoyi"];
//    [BIMCacheObject delete_Data_urlKey:@"touzhu"];
//    [BIMCacheObject delete_Data_urlKey:@"jixian"];
}
...
//
//    
//    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:@"http://mobile.gunqiu.com/interface/version" Start:^(id requestOrignal) {
//        
//    } End:^(id responseOrignal) {
//        
//    } Success:^(id responseResult, id responseOrignal){
...
//
//    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_loginAndRegister] Start:^(id requestOrignal) {
//        
//    } End:^(id responseOrignal) {
//        
//    } Success:^(id responseResult, id responseOrignal){
...
- (void)loadLaunchImageView {
    NSMutableDictionary *parameter = [NSMutableDictionary dictionaryWithDictionary:[BIMHttpString getCommenParemeter] ];
    [[BIMDCHttpRequest shareInstance] sendGetRequestByMethod:@"get" WithParamaters:parameter PathUrlL:[NSString stringWithFormat:@"%@%@",APPDELEGATE.url_Server,url_startpage] Start:^(id requestOrignal) {
        ...
        if ([[responseOrignal objectForKey:@"code"] isEqualToString:@"200"]) {
            ...
            if (!(dataDic.allKeys.count == 0)) {
                BIMLaunchView *launchV = [[BIMLaunchView alloc] initWithFrame:self.window.bounds];
                launchV.dataDic = dataDic;
                [[BIMMethods getMainWindow] addSubview:launchV];
            }
        }
    } Failure:^(NSError *error, NSString *errorDict, id responseOrignal) {
         [self loadConfig];
